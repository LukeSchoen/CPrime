...
int main(int argc, char **argv) {
    ...
    nt_join(clang, sizeof clang, root, "src\\third-party\\clang\\bin\\clang.exe");
    nt_join(out, sizeof out, root, "src\\build\\clang");
    for (i = 1; i < argc; ++i) {
...
#define ADD(x) args[n++] = (x)
    ADD(clang); ADD("-I"); { static char p[NT_PATH]; nt_join(p,sizeof p,root,"src\\include\\cprime"); ADD(p); }
