...
#define EM_CPRIME_TARGET EM_X86_64
...
/* Returns an enumerator to describe whether and when the relocation needs a
   GOT and/or PLT entry to be created. See cpc.h for a description of the
   different values. */
...
ST_FUNC unsigned create_plt_entry(CPRIMEState *s1, unsigned got_offset, struct sym_attr *attr)
{
  ...
  write32le(p + 7, relofs / sizeof (ObjW_Rel) - 1);
...
ST_FUNC void relocate_plt(CPRIMEState *s1)
{
  ...
  if (s1->plt->reloc)
  {
    ...
    p = s1->got->data;
    for_each_elem(s1->plt->reloc, 0, rel, ObjW_Rel)
    {
...
ST_FUNC void relocate(CPRIMEState *s1, ObjW_Rel *rel, int type, unsigned char *ptr, addr_t addr, addr_t val)
{
  ...
  case R_X86_64_64:
    if (s1->output_type & CPRIME_OUTPUT_DYN)
    {
      ...
      if (esym_index)
      {
        qrel->r_info = Obj64_R_INFO(esym_index, R_X86_64_64);
      ...
      {
        qrel->r_info = Obj64_R_INFO(0, R_X86_64_RELATIVE);
  ...
  case R_X86_64_32S:
    if (s1->output_type & CPRIME_OUTPUT_DYN)
    {
      /* XXX: this logic may depend on CPRIME's codegen
         now CPRIME uses R_X86_64_32 even for a 64bit pointer */
      qrel->r_offset = rel->r_offset;
      qrel->r_info = Obj64_R_INFO(0, R_X86_64_RELATIVE);
      // Use sign extension!
    ...
    if ((type == R_X86_64_32 ? val != (unsigned)val : val != (int)val))
      cprime_error_noabort("relocation 'R_X86_64_32[S]' out of range");
    add32le(ptr, val);
    break;
  ...
  case R_X86_64_PC32:
    if (s1->output_type == CPRIME_OUTPUT_DLL)
    {
      ...
      {
        qrel->r_offset = rel->r_offset;
        qrel->r_info = Obj64_R_INFO(esym_index, R_X86_64_PC32);
        // Use sign extension!
...
plt32pc32:
    {
      ...
      if (diff < -2147483648LL || diff > 2147483647LL)
      {
        ...
        if (((ObjW(Sym) * )symtab_section->data)[sym_index].st_shndx != SHN_UNDEF)
#endif
          cprime_error_noabort("relocation '%d' out of range", type);
      }
      add32le(ptr, diff);
    }
    break;
  ...
  case R_X86_64_PC64:
    if (s1->output_type == CPRIME_OUTPUT_DLL)
    {
      ...
      {
        qrel->r_offset = rel->r_offset;
        qrel->r_info = Obj64_R_INFO(esym_index, R_X86_64_PC64);
  ...
  case R_X86_64_TLSGD:
  {
    ...
    if (memcmp (ptr - 4, expect, sizeof(expect)) == 0)
    {
      ...
      rel[1].r_info = Obj64_R_INFO(0, R_X86_64_NONE);
      ...
      cprime_error_noabort("unexpected R_X86_64_TLSGD pattern");
  }
  break;
  case R_X86_64_TLSLD:
  {
    ...
    if (memcmp (ptr - 3, expect, sizeof(expect)) == 0)
    {
      ...
      cprime_error_noabort("unexpected R_X86_64_TLSLD pattern");
  }
  break;
  case R_X86_64_DTPOFF32:
  case R_X86_64_TPOFF32:
  ...
  case R_X86_64_RELATIVE:
    ...
    break;
  default:
    fprintf(stderr, "Unhandled reloc type %d at %x [%p] to %x\n",
