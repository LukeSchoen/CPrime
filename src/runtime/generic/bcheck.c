...
#define HAVE_TLS_FUNC          (1)
#define HAVE_TLS_VAR           (0)
#if defined CONFIG_CPRIME_MUSL || defined __ANDROID__
# undef HAVE_CTYPE
#endif
#endif
...
#define CPRIME_TYPE_NONE           (0)
#define CPRIME_TYPE_MALLOC         (1)
#define CPRIME_TYPE_CALLOC         (2)
#define CPRIME_TYPE_REALLOC        (3)
#define CPRIME_TYPE_MEMALIGN       (4)
#define CPRIME_TYPE_STRDUP         (5)
...
int cprime_backtrace(const char *fmt, ...);

/* print a bound error message */
#define bound_warning(...) \
    do {                                                 \
        WAIT_SEM ();                                     \
        cprime_backtrace("^bcheck.c^BCHECK: " __VA_ARGS__); \
        POST_SEM ();                                     \
    } while (0)
...
#define bounds_loc(fp, ...) \
    do {                            \
        WAIT_SEM (); \
        cprime_backtrace("^bcheck.c^\001" __VA_ARGS__); \
        POST_SEM (); \
    } while (0)
...
/* Needed when using ...libcprime1-usegcc=yes in lib/Makefile */
...
void __bound_init(size_t *p, int mode)
{
    ...
    print_warn_ptr_add = getenv ("CPRIME_BOUNDS_WARN_POINTER_ADD") != NULL;
    print_calls = getenv ("CPRIME_BOUNDS_PRINT_CALLS") != NULL;
    print_heap = getenv ("CPRIME_BOUNDS_PRINT_HEAP") != NULL;
    print_statistic = getenv ("CPRIME_BOUNDS_PRINT_STATISTIC") != NULL;
    never_fatal = getenv ("CPRIME_BOUNDS_NEVER_FATAL") != NULL;
...
#if MALLOC_REDIR
    {
        ...
        /* cpc -run required RTLD_DEFAULT. Normal usage requires RTLD_NEXT,
...
void __attribute__((destructor)) __bound_exit(void)
{
    ...
    if (inited) {
#if !defined(_WIN32) && !defined CONFIG_CPRIME_MUSL && !defined(__ANDROID__)
        if (print_heap) {
...
#endif
{
    ...
    if (NO_CHECKING_GET() == 0) {
        ...
        if (ptr) {
            tree = splay_insert((size_t) ptr, size ? size : size + 1, tree);
            if (tree && tree->start == (size_t) ptr)
                tree->type = CPRIME_TYPE_MEMALIGN;
        }
        POST_SEM ();
    }
    return ptr;
