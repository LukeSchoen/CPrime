...
static void archive_replay(void)
{
  ...
  for (i = 0; i < 4; ++i) {
    FILE *file;
    sprintf(path, "build/cold-%s.c", names[i]);
    file = fopen(path, "wb");
    CHECK(file != NULL);
    if (!file) return;
  ...
  for (i = 0; i < 3; ++i) {
    sprintf(path, "build/cold-%s", products[i]); remove(path);
  }
}
...
int main(void)
{
  ...
  int provider, i, fd;
  unsigned generation;
  const char *path = "build/cold-paths.def";
  ...
  for (i = 0; i < 3; ++i) {
    state = cprime_new();
    cprime_add_include_path(state, "include/runtime");
    CHECK(cprime_set_output_type(state, CPRIME_OUTPUT_OBJ) == 0);
