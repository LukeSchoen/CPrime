...
%CC% %HOST_OPT% -o cpc.exe %CPRIME_C% %EXTRA_BOOT_SRCS% %D% %EXTRA_LINK_LIBS%
@if errorlevel 1 goto :the_end
@goto :compiler_done
...
copy>nul %ROOT%\include\runtime\* include
...
@rem Rebuild with the selected bootstrap compiler.  Its predefined target CRT
@rem controls the startup ABI, which may differ from the compiler being built.
copy /y>nul %ROOT%\third-party\win32-sdk\lib\*.def %ROOT%\lib
...
if errorlevel 1 exit /B 1
%CC% -m%T% -c %ROOT%\src\runtime\windows\wincompat.c
if errorlevel 1 exit /B 1
%CC% -m%T% -I%ROOT%\include\runtime -I%ROOT%\third-party\quickjs-regexp -c %ROOT%\src\runtime\windows\regex.c
if errorlevel 1 exit /B 1
%CC% -m%T% -c %ROOT%\third-party\quickjs-regexp\cutils.c
if errorlevel 1 exit /B 1
%CC% -m%T% -c %ROOT%\third-party\quickjs-regexp\libunicode.c
if errorlevel 1 exit /B 1
%CC% -m%T% -c %ROOT%\third-party\quickjs-regexp\libregexp.c
if errorlevel 1 exit /B 1
%CC% -m%T% -c %ROOT%\src\runtime\windows\ucrt_stdio.c
if errorlevel 1 exit /B 1
%CC% -m%T% -c %ROOT%\src\runtime\windows\ucrt_exit.c
if errorlevel 1 exit /B 1
%CC% -m%T% -c %ROOT%\src\runtime\windows\ucrt_onexit.c
if errorlevel 1 exit /B 1
...
if errorlevel 1 exit /B 1
%CC% -I%ROOT%\include\runtime -m%T% -c %ROOT%\src\runtime\windows\new_delete.cpp
if errorlevel 1 exit /B 1
...
:make_lib
if not exist lib mkdir lib
copy /y>nul %ROOT%\third-party\win32-sdk\lib\*.def lib
...
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -m%1 -c %ROOT%\src\runtime\windows\wincompat.c
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -m%1 -I%ROOT%\include\runtime -I%ROOT%\third-party\quickjs-regexp -c %ROOT%\src\runtime\windows\regex.c
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -m%1 -c %ROOT%\third-party\quickjs-regexp\cutils.c
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -m%1 -c %ROOT%\third-party\quickjs-regexp\libunicode.c
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -m%1 -c %ROOT%\third-party\quickjs-regexp\libregexp.c
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -m%1 -c %ROOT%\src\runtime\windows\ucrt_stdio.c
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -m%1 -c %ROOT%\src\runtime\windows\ucrt_exit.c
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -m%1 -c %ROOT%\src\runtime\windows\ucrt_onexit.c
if errorlevel 1 exit /B 1
...
if errorlevel 1 exit /B 1
.\cpc -B. %INC_FLAGS% -I%ROOT%\include\runtime -m%1 -c %ROOT%\src\runtime\windows\new_delete.cpp
if errorlevel 1 exit /B 1
