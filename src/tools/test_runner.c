...
static char root[NT_PATH], tests_root[NT_PATH], work_root[NT_PATH];
...
static int run_regressions(const char *compiler, const char *runtime, unsigned timeout_ms) {
    ...
    snprintf(work, sizeof work, "%s\\native-regression-%lu-%u", work_root,
...
static int run_checks(const char *compiler, const char *runtime, unsigned timeout_ms) {
    ...
    nt_join(maintenance, sizeof maintenance, root, "src\\scripts\\maintenance.exe");
    ...
    nt_process_free(&result);
    nt_join(helper_source, sizeof helper_source, root, "src\\tools\\test_compiler_cold_paths.c");
    nt_join(helper_exe, sizeof helper_exe, work_root, "test-compiler-cold-paths.exe");
    {
...
#define ROOT_INCLUDE(slot, relative) do { char p[NT_PATH]; nt_join(p,sizeof p,root,relative); snprintf(slot,sizeof slot,"-I%s",p); compile[n++]=slot; } while (0)
        ...
        ROOT_INCLUDE(i1,"src\\include\\runtime"); ROOT_INCLUDE(i2,"src\\include\\cprime");
        ROOT_INCLUDE(i3,"src\\third-party\\win32-sdk\\include"); ROOT_INCLUDE(i4,"src\\third-party\\win32-sdk\\include\\winapi");
    ...
    nt_join(helper_source, sizeof helper_source, root, "src\\tools\\test_process_path.c");
    nt_join(helper_exe, sizeof helper_exe, work_root, "test-process-path.exe");
    {
...
static int run_all_suites(const char *compiler, const char *runtime, const char *tier,
    ...
    /* Each area keeps its own tests directory: a suite is any directory under
       the tests root that holds pass/ or fail/ cases, plus every
       features/<name> directory. Directories this tree does not have are
       simply not part of the run. */
    {
        char top_pattern[NT_PATH];
        nt_join(top_pattern, sizeof top_pattern, tests_root, "*");
        find = FindFirstFileA(top_pattern, &entry);
        if (find != INVALID_HANDLE_VALUE) {
            do {
                char child[NT_PATH], probe[NT_PATH];
                if (!(entry.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) continue;
                if (!strcmp(entry.cFileName, ".") || !strcmp(entry.cFileName, "..")) continue;
                if (!_stricmp(entry.cFileName, "features") || !_stricmp(entry.cFileName, "build") ||
                    !_stricmp(entry.cFileName, "tools") || !_stricmp(entry.cFileName, "include"))
                    continue;
                nt_join(child, sizeof child, tests_root, entry.cFileName);
                nt_join(probe, sizeof probe, child, "pass");
                if (!nt_exists(probe)) {
                    nt_join(probe, sizeof probe, child, "fail");
                    if (!nt_exists(probe)) continue;
                }
                count = add_suite(suites, count, entry.cFileName);
            } while (FindNextFileA(find, &entry));
            FindClose(find);
        }
    }
...
int main(int argc, char **argv) {
    ...
    /* Evidence belongs to the area that owns these tests, so it lands in the
       sibling build/ directory rather than inside the retained test tree. */
    strcpy(work_root, tests_root);
    nt_parent(work_root);
    nt_join(work_root, sizeof work_root, work_root, "build");
