...
static CPRIMEState *g_s1;
// Semaphore To Protect It
CPRIME_SEM(static rt_sem);
...
#ifndef CONFIG_CPRIME_BACKTRACE_ONLY
...
static int cprime_relocate_ex(CPRIMEState *s1, void *ptr, unsigned ptr_diff);
static void st_link(CPRIMEState *s1);
static void st_unlink(CPRIMEState *s1);
#ifdef CONFIG_CPRIME_BACKTRACE
static int _cprime_backtrace(rt_frame *f, const char *fmt, va_list ap);
#endif
#ifdef _WIN64
static void *win64_add_function_table(CPRIMEState *s1);
...
static int rt_mem(CPRIMEState *s1, int size)
{
  void *ptr;
  int ptr_diff = 0;
  ...
  void *prw;
  char tmpfname[] = "/tmp/.cprimerunXXXXXX";
  ...
  if (ptr == MAP_FAILED || prw == MAP_FAILED)
    return cprime_error_noabort("cprimerun: could not map memory");
  ptr_diff = (char *)prw - (char *)ptr; // = size;
  //printf("map %p %p %p\n", ptr, prw, (void*)ptr_diff);
  size *= 2;
#else
  ptr = cprime_malloc(size += PAGESIZE); // One Extra Page To Align Malloc Memory
#endif
  s1->run_ptr = ptr;
  s1->run_size = size;
  return ptr_diff;
...
// -------------------------------------------------------------
/* Do all relocations (needed before using cprime_get_symbol())
   Returns -1 on error. */
...
LIBCPRIMEAPI int cprime_relocate(CPRIMEState *s1)
{
  ...
  if (s1->run_ptr)
    exit(cprime_error_noabort("'cprime_relocate()' twice is no longer supported"));
#ifdef CONFIG_CPRIME_BACKTRACE
  if (s1->do_backtrace)
    cprime_add_symbol(s1, "_cprime_backtrace", _cprime_backtrace); // For Bt-Log.C
#endif
  size = cprime_relocate_ex(s1, NULL, 0);
  if (size < 0)
    return -1;
  ptr_diff = rt_mem(s1, size);
  if (ptr_diff < 0)
    return -1;
  ret = cprime_relocate_ex(s1, s1->run_ptr, ptr_diff);
  if (ret == 0)
    st_link(s1);
  return ret;
}
...
LIBCPRIMEAPI int cprime_run(CPRIMEState *s1, int argc, char **argv)
{
  ...
  // cpc -dt -run ... nothing to do if no main()
  if ((s1->dflag & 16) && (addr_t) -1 == get_sym_addr(s1, "main", 0, 1))
    return 0;

  cprime_add_symbol(s1, "__rt_exit", rt_exit);
  s1->run_main = "_runmain", top_sym = "main";
  if (s1->elf_entryname)
    s1->run_main = top_sym = s1->elf_entryname;
  cprime_add_support(s1, "runmain.o");

  if (cprime_relocate(s1) < 0)
    return -1;
  ...
  {
    cprime_error_noabort("failed to reopen stdin from '%s'", s1->run_stdin);
    return -1;
  }
  ...
  ret = cprime_setjmp(s1, main_jb, cprime_get_symbol(s1, top_sym));
  if (0 == ret)
    ret = prog_main(argc, argv, envp);
  else if (RT_EXIT_ZERO == ret)
    ret = 0;

  if (s1->dflag & 16 && ret) // Cpc -Dt -Run ...
...
static void cleanup_symbols(CPRIMEState *s1)
{
  Section *s = s1->symtab;
  int sym_index, end_sym = s->data_offset / sizeof (ObjSym);
  // Reset Symtab
  ...
  {
    ObjW(Sym) *sym = &((ObjW(Sym) *)s->data)[sym_index];
...
static void cleanup_sections(CPRIMEState *s1)
{
  ...
  do
  {
    for (i = --f; i < p->nb_secs; i++)
    {
      ...
      if (s1->do_debug || s == s1->symtab || s == s1->symtab->link || s == s1->symtab->hash)
        s->data = cprime_realloc(s->data, s->data_allocated = s->data_offset);
      else
        free_section(s), cprime_free(s), p->secs[i] = NULL;
    }
  }
  while (++p, f);
}
...
# if (defined CPRIME_TARGET_ARM && !TARGETOS_BSD) || defined CPRIME_TARGET_ARM64
  if (mode == 0 || mode == 3)
  {
...
static void bt_link(CPRIMEState *s1)
{
#ifdef CONFIG_CPRIME_BACKTRACE
  rt_context *rc;
  if (!s1->do_backtrace)
    return;
  rc = cprime_get_symbol(s1, "__rt_info");
  if (!rc)
    return;
  rc->esym_start = (ObjW(Sym) *)(symtab_section->data);
  rc->esym_end = (ObjW(Sym) *)(symtab_section->data + symtab_section->data_offset);
  ...
  {
    void *p;
    if ((p = cprime_get_symbol(s1, "__bound_init")))
...
#endif
  ptr_unlink(&g_s1, s1, offsetof(CPRIMEState, next));
  rt_post_sem();
}

LIBCPRIMEAPI void *_cprime_setjmp(CPRIMEState *s1, void *p_jmp_buf, void *func, void *p_longjmp)
{
  s1->run_lj = p_longjmp;
  s1->run_jb = p_jmp_buf;
#ifdef CONFIG_CPRIME_BACKTRACE
...
LIBCPRIMEAPI void cprime_set_backtrace_func(CPRIMEState *s1, void *data, CPRIMEBtFunc *func)
{
  s1->bt_func = func;
  s1->bt_data = data;
}

static CPRIMEState *rt_find_state(rt_frame *f)
{
  CPRIMEState *s;
  int level;
  addr_t pc;
...
static void rt_exit(rt_frame *f, int code)
{
  ...
  if (s && s->run_lj)
  {
    ...
    {
      void *p = cprime_get_symbol(s, "__bound_exit");
      if (p)
        ((void (*)(void))p)();
    }
#endif
...
// -------------------------------------------------------------
#else // if defined CONFIG_CPRIME_BACKTRACE_ONLY
static void rt_exit(rt_frame *f, int code)
{
  exit(code);
}
#endif // ndef CONFIG_CPRIME_BACKTRACE_ONLY
// -------------------------------------------------------------
#ifdef CONFIG_CPRIME_BACKTRACE
...
static char *rt_elfsym(rt_context *rc, addr_t wanted_pc, addr_t *func_addr)
{
  ...
  {
    int type = Obj64_ST_TYPE(esym->st_info);
...
static
#endif
int _cprime_backtrace(rt_frame *f, const char *fmt, va_list ap)
{
  rt_context *rc, *rc2;
  addr_t pc;
  ...
  one = 0;
  // Leading \001 forces Dprintf() to emit a single-line message.
  if (fmt[0] == '\001')
    ++fmt, one = 1;
  ...
  rt_wait_sem();
  rc = g_rc;
  getinfo = rt_printline_dwarf, n = 6;
  if (rc)
  {
    if (rc->num_callers)
      n = rc->num_callers;
...
static int rt_error(rt_frame *f, const char *fmt, ...)
{
  ...
  ret = _cprime_backtrace(f, msg, ap);
  va_end(ap);
  return ret;
}
...
static void set_exception_handler(void)
{
  ...
  /* install CPRIME signal handlers to print debug info on fatal
     runtime errors */
...
#endif
#else // for runmain.c:exit(); when CONFIG_CPRIME_BACKTRACE == 0 */
static int rt_get_caller_pc(addr_t *paddr, rt_frame *f, int level)
{
  if (level)
    return -1;
...
static CPRIMESyms cprime_syms[] =
{
#if !defined(CONFIG_CPRIMEBOOT)
#define CPRIMESYM(a) { #a, &a, },
  CPRIMESYM(printf)
  CPRIMESYM(fprintf)
  CPRIMESYM(fopen)
  CPRIMESYM(fclose)
...
#endif // CONFIG_CPRIME_STATIC 
#endif // CPRIME_IS_NATIVE 
// -------------------------------------------------------------
