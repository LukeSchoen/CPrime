#ifndef _CPRIME_LIBM_H_
#define _CPRIME_LIBM_H_
...
#define CPRIMEFP_FORCE_EVAL(x) do {  \
...
__CRT_INLINE double __cdecl asinh(double x) {
  ...
  else CPRIMEFP_FORCE_EVAL(x + 0x1p120f);
  return s ? -x : x;
}
...
__CRT_INLINE double __cdecl atanh(double x) {
  ...
  u.i &= -1ull / 2, x = u.f;
  if (e < 0x3ff - 1) {
    if (e < 0x3ff - 32) { if (e == 0) CPRIMEFP_FORCE_EVAL((float)x); }
