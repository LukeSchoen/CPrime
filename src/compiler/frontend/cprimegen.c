...
static void decl_initializer_alloc(CType *type, AttributeDef *ad, int r, int has_init,
                                   int has_ctor_init, int v, int decl_scope);
...
static int try_call_cpp_compound_add_assign_operator(void);
...
static int instantiate_template_if_needed(TemplateDef *td, int type_tok);
...
static int make_free_func_tok_for_type(int name_tok, CType *func_type)
{
  ...
  if (!is_cpp_translation_unit())
    return name_tok;
  if (strstr(get_tok_str(name_tok, NULL), "__"))
    return name_tok;
...
static Sym *resolve_free_func_by_arg_count(int name_tok, int explicit_arg_count)
{
  ...
  for (o = free_func_overloads; o; o = o->next)
  {
    if (o->name_tok != name_tok || o->explicit_arg_count != explicit_arg_count)
      continue;
                                           ...
                                           int explicit_arg_count)
{
  ...
  for (o = free_func_overloads; o; o = o->next)
  {
    ...
    {
      if (!arg || !is_compatible_unqualified_types(&arg->type, &arg_types[i]))
      {
        ok = 0;
        break;
      }
                                              ...
                                              TokenString *params)
{
  ...
  for (o = member_func_overloads; o; o = o->next)
        ...
        && o->explicit_arg_count == param_count)
    {
      ...
      other_count++;
  if (same_count == 1 && !other_count)
    return same_mangled_tok;
  if (!sym_find(tok_alloc_const(name)) && !sym_find2(global_stack, tok_alloc_const(name)))
    return tok_alloc_const(name);
  for (i = 0; i < params->len && params->str[i] != TOK_EOF; ++i)
  {
    switch (params->str[i])
...
{
  Sym *s = resolve_lifecycle_func(type, TOK_CONSTRUCTOR1);
  if (!lifecycle_has_only_this_param(s))
    return NULL;
  return s;
}
                                       ...
                                       int call_arg_count)
{
  ...
  if (!ctor_func || (ctor_func->type.t & VT_BTYPE) != VT_FUNC || !ctor_func->type.ref)
    cprime_error("placement new requires a declared constructor");
...
static int try_parse_cpp_placement_new_after_name(void)
{
  ...
  if ((type.t & VT_BTYPE) != VT_STRUCT)
    cprime_error("placement new requires a class or struct type");
  ...
  else if (!resolve_lifecycle_func(&type, TOK_CONSTRUCTOR1))
  {
    int saved_tok = tok;
                                             ...
                                             int explicit_arg_count)
{
  ...
  for (o = member_func_overloads; o; o = o->next)
  {
    ...
    if (!s)
      continue;
    if (match && match != s)
      cprime_error("ambiguous overloaded member function '%s'",
                get_tok_str(method_tok, NULL));
    match = s;
  }
  if (match)
                                         ...
                                         int explicit_arg_count)
{
  ...
  for (i = 0; i < explicit_arg_count; ++i)
  {
    if (!arg)
      return 0;
    if (!compare_types(&arg->type, &arg_types[i], 1))
      return 0;
    arg = arg->next;
  }
  return arg == NULL;
                                       ...
                                       TokenString *body, int internal)
{
  ...
  mangled_tok = make_lifecycle_func_tok_for_params(struct_tok, method_tok, params);
...
static TemplateDef *find_template_def(int name_tok)
{
  ...
  for (i = 0; i < nb_template_defs; ++i)
  {
    if (template_defs[i]->name_tok == name_tok)
...
static int find_template_member_class_in_str(TokenString *str, int type_param_tok)
{
  ...
  for (i = 0; i + 6 < n; ++i)
  {
    if (str->str[i] >= TOK_UIDENT
        && str->str[i + 1] == TOK_LT
        && str->str[i + 2] == type_param_tok
        && str->str[i + 3] == TOK_GT
        && str->str[i + 4] == ':'
        && str->str[i + 5] == ':'
        && str->str[i + 6] >= TOK_UIDENT)
      return str->str[i];
    if (i + 7 < n
        && str->str[i] >= TOK_UIDENT
        && str->str[i + 1] == TOK_LT
        && str->str[i + 2] == type_param_tok
        && str->str[i + 3] == TOK_GT
        && str->str[i + 4] == ':'
...
static void skip_member_func_cv_qualifiers(void)
{
...
{
  int saved_tok, method_tok, paren, i;
  CValue saved_tokc;
  TokenString *ret_str, *sig, *proto, *body_def;
  PendingMemberFunc *pm;
  ...
  for (;;)
  {
    ...
    tok_str_add_template_subst_plain(ret_str, md->type_param_tok, type_tok);
    next();
  }
  ...
  else
  {
    ...
    {
      int rt = ret_str->str[i];
      tok_str_add(sig, rt == md->type_param_tok ? type_tok : rt);
    }
  }

  tok_str_add(sig, make_member_func_tok(class_mangled_tok, method_tok));
  tok_str_add(sig, '(');
  ...
  if (tok != ')')
  {
    ...
    for (;;)
    {
      ...
      if (tok == '(')
        ...
        --paren;
      tok_str_add_template_subst_plain(sig, md->type_param_tok, type_tok);
      next();
    }
  }
  skip(')');
  ...
  while (tok != TOK_EOF)
  {
    tok_str_add_template_subst(body_def, md->type_param_tok, type_tok);
    next();
  }
                                    ...
                                    TokenString *str)
{
  ...
  for (i = 0; i < td->nb_inst; ++i)
    instantiate_template_member_if_needed(md, td->inst_type_toks[i],
                                          td->inst_name_toks[i]);
}

static void parse_template_decl(void)
{
  int type_param_tok, name_tok = 0, member_class_tok, prev_ident = 0, prev_tok = 0;
  ...
  if (tok < TOK_UIDENT)
    expect("typename");
  kw = get_tok_str(tok, NULL);
  if (0 != strcmp(kw, "typename"))
    cprime_error("expected 'typename' in template parameter list");
  next();
  if (tok < TOK_UIDENT)
    expect("identifier");
  type_param_tok = tok;
  next();
  if (tok == ',')
    cprime_error("only one template type parameter is supported");
  skip(TOK_GT);
  ...
  member_class_tok = find_template_member_class_in_str(str, type_param_tok);
  if (member_class_tok)
  {
    add_template_member_def(member_class_tok, type_param_tok, str);
    return;
  }

  if (find_template_def(name_tok))
    cprime_error("redefinition of template '%s'", get_tok_str(name_tok, NULL));

  td = cprime_mallocz(sizeof(*td));
  td->name_tok = name_tok;
  td->type_param_tok = type_param_tok;
...
static int template_lookup_inst(TemplateDef *td, int type_tok)
{
  int i;
  for (i = 0; i < td->nb_inst; ++i)
    if (td->inst_type_toks[i] == type_tok)
...
static int template_subst_token(TemplateDef *td, int type_tok,
                                int class_mangled_tok,
...
static void template_add_subst_token(TokenString *spec, TemplateDef *td,
                                     int type_tok, int class_mangled_tok,
                                     int *alias_from, int *alias_to,
                                     int nb_aliases, int t)
{
  int subst_tok;

  subst_tok = template_subst_token(td, type_tok, class_mangled_tok,
...
static int parse_template_type_arg(void)
{
  ...
  skip(TOK_LT);
  if (tok == TOK_INT || tok == TOK_LONG || tok == TOK_FLOAT)
  {
    type_tok = tok;
    next();
  }
  ...
  {
    TemplateDef *td = find_template_def(tok);
    if (td && td->is_class)
    {
      int nested_type_tok, nested_mangled_tok;
      next();
      if (tok != TOK_LT)
        cprime_error("unsupported template type argument");
      nested_type_tok = parse_template_type_arg();
      nested_mangled_tok = instantiate_template_if_needed(td, nested_type_tok);
      compile_pending_template_specs();
...
static int instantiate_template_if_needed(TemplateDef *td, int type_tok)
{
  ...
  mangled_tok = template_lookup_inst(td, type_tok);
  if (mangled_tok)
    return mangled_tok;

  sanitize_type_suffix(typebuf, sizeof(typebuf), get_tok_str(type_tok, NULL));
  snprintf(name, sizeof(name), "%s__%s", get_tok_str(td->name_tok, NULL), typebuf);
  mangled_tok = tok_alloc_const(name);
  ...
  while (tok != TOK_EOF)
  {
    ...
    if (td->is_class && tok == TOK_TYPEDEF)
    {
      ...
      for (i = 0; i < typedef_str->len; ++i)
      {
        ...
        else if (t == td->type_param_tok)
          template_add_subst_token(spec, td, type_tok, mangled_tok,
                                   alias_from, alias_to, nb_aliases, t);
        else
          tok_str_add(spec, template_subst_token(td, type_tok, mangled_tok,
                                                 alias_from, alias_to,
                                                 nb_aliases, t));
      }
  ...
  if (td->nb_inst >= td->al_inst)
  {
    ...
    td->inst_type_toks = cprime_realloc(td->inst_type_toks, td->al_inst *sizeof(int));
    td->inst_name_toks = cprime_realloc(td->inst_name_toks, td->al_inst *sizeof(int));
  }
  td->inst_type_toks[td->nb_inst] = type_tok;
  td->inst_name_toks[td->nb_inst] = mangled_tok;
  td->nb_inst++;
  instantiate_template_members_for_class(td, type_tok, mangled_tok);
  return mangled_tok;
}
...
static int try_parse_cpp_lifecycle_def(void)
{
  ...
  CType struct_type;
  Sym *s;
  TokenString *body = NULL, *params = NULL;
  TokenString *replay;
...
{
  int method_tok, saved_tok, paren, i, mangled_tok;
  int has_params = 0;
  CValue saved_tokc;
  Sym *class_sym, *decl_field, *canonical_sym;
  CType *effective_ret_type;
  CType class_type, lowered_type;
  int dummy_ofs;
...
enum_done:
    ...
    else
    {
      ...
      while (tok != '}')
      {
        ...
        if (tok == get_struct_type_name_tok(type) || tok == '~')
        {
        ...
        while (1)
        {
          ...
          if (tok != ';')
          {
            ...
            if ((type1.t & VT_BTYPE) == VT_FUNC)
            {
              ...
              if (tok == ';')
              {
                declare_member_func(type, v, &type1);
                member_func_body = 1;
                break;
              }
            ...
            if (type1.t & VT_STATIC)
            {
              ...
              if (tok == '=')
                cprime_error("static data member '%s' must be defined out of class",
                          get_tok_str(v, NULL));
    ...
    default:
      if (!typespec_found)
      {
        ...
        if (td && td->is_class)
        {
          ...
          type_tok = parse_template_type_arg();
          mangled_tok = instantiate_template_if_needed(td, type_tok);
          compile_pending_template_specs();
...
str_init:
    ...
    decl_initializer_alloc(&type, &ad, VT_CONST, 2, 0, 0, 0);
    break;
  case TOK_SOTYPE:
  case '(':
    t = tok;
    ...
    if (parse_btype(&type, &ad, 0))
    {
      ...
      if (tok == '{')
      {
        ...
        if (!(type.t & VT_ARRAY))
          r |= VT_LVAL;
        memset(&ad, 0, sizeof(AttributeDef));
        decl_initializer_alloc(&type, &ad, r, 1, 0, 0, 0);
      }
...
tok_identifier:
    {
      ...
      if (tok == ':')
      {
        ...
        else
        {
          ...
          if (tok < TOK_UIDENT)
            cprime_error("static data member name");
          t = make_static_member_tok(class_tok, tok);
          next();
        }
      }
      n = 0;
        ...
        break;
      {
        ...
        if (td && !td->is_class && tok == '(')
        {
          int type_tok, inferred_call;
          next();
          inferred_call = 0;
          if (tok == TOK_INT || tok == TOK_LONG || tok == TOK_FLOAT)
          {
          ...
          {
            type_tok = infer_template_type_tok_from_first_arg();
            if (!type_tok)
          ...
          t = instantiate_template_if_needed(td, type_tok);
          s = sym_find(t);
          if (!s)
  ...
  while (1)
  {
    ...
    else if (tok == '.' || tok == TOK_ARROW)
    {
      ...
      if (!field && tok == '(' && (vtop->type.t & VT_BTYPE) == VT_STRUCT)
      {
...
static void expr_eq(void)
{
  ...
  if ((t = tok) == '=' || TOK_ASSIGN(t))
  {
    ...
    else
    {
      vdup();
      expr_eq();
      if (t == TOK_A_ADD && try_call_cpp_compound_add_assign_operator())
      {
        vswap();
        vpop();
        return;
...
static void decl_initializer_alloc(CType *type, AttributeDef *ad, int r,
                                   int has_init, int has_ctor_init, int v, int decl_scope)
{
  int size, align, addr;
...
err_size:
  ...
  if ((r & VT_VALMASK) == VT_LOCAL && sym && !NODATA_WANTED && !(type->t & VT_VLA))
  {
    ...
    if (ctor_func && has_ctor_init)
      call_lifecycle_constructor(&sym->type, sym->r, sym->c, sym, ctor_func);
    else if (ctor_func && !has_expr_struct_init)
    {
      save_lvalues();
      vpushsym(&ctor_func->type, ctor_func);
        ...
        else
        {
          ...
          else
          {
            ...
            if (has_init)
              ...
              type.t |= VT_EXTERN;
            decl_initializer_alloc(&type, &ad, r, has_init, has_ctor_init, v, l);
          }
