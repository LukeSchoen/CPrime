...
void Racer()
{
  ...
  while (clControls::Update())
  {
    ...
    camZoom = 0.01f + (camCenter - car.m_pos).Length() * 8;
    viewArea.GrowUniform(camZoom);
    ...
    if (spinCamera)
      camera.LookAt(camera.GetPosition(), clVec3D(clVec2D(car.m_pos), 0.f));

    clMat4 VP(camera.ViewProjectionMatrix());
}
