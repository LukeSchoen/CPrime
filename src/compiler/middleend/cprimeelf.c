...
#if CPRIME_EH_FRAME
  if (s->output_format != CPRIME_OUTPUT_FORMAT_ELF)
    s->unwind_tables = 0;
  cprime_eh_frame_start(s);
#endif
...
#ifdef CPRIME_TARGET_PE
  /* to make sure that -lcprime1 -Wl,-e,_start will grab the startup code
     from libcprime1.a (unless _start defined) */
  if (s->elf_entryname)
...
ST_FUNC void cprimeelf_delete(CPRIMEState *s1)
{
  ...
  {
    cprime_free(sym_versions[i].version);
    cprime_free(sym_versions[i].lib);
  }
  cprime_free(sym_versions);
  cprime_free(sym_to_version);
#endif
  ...
  cprime_free(s1->sym_attrs);
#ifdef CPRIME_TARGET_RISCV64
  cprime_free(s1->pcrel_hi_entries);
#endif
...
static void update_relocs(CPRIMEState *s1, Section *s, int *old_to_new_syms, int first_sym);
...
ST_FUNC void cprimeelf_end_file(CPRIMEState *s1)
{
  ...
  first_sym = s->sh_offset / sizeof (ObjSym);
  nb_syms = s->data_offset / sizeof (ObjSym) - first_sym;
  s->data_offset = s->sh_offset;
  s->link->data_offset = s->link->sh_offset;
  s->hash = s->reloc, s->reloc = NULL;
  tr = cprime_mallocz(nb_syms *sizeof *tr);

  for (i = 0; i < nb_syms; ++i)
  {
    ObjSym *sym = (ObjSym *)s->data + first_sym + i;
    if (sym->st_shndx == SHN_UNDEF)
    {
      int sym_bind = Obj64_ST_BIND(sym->st_info);
      int sym_type = Obj64_ST_TYPE(sym->st_info);
      if (sym_bind == STB_LOCAL)
        sym_bind = STB_GLOBAL;
#ifndef CPRIME_TARGET_PE
      if (sym_bind == STB_GLOBAL && s1->output_type == CPRIME_OUTPUT_OBJ)
      {
...
#endif
      sym->st_info = Obj64_ST_INFO(sym_bind, sym_type);
    }
...
ST_FUNC Section *new_section(CPRIMEState *s1, const char *name, int sh_type, int sh_flags)
{
  ...
  sec = cprime_mallocz(sizeof(Section) + strlen(name));
  sec->s1 = s1;
...
ST_FUNC void init_symtab(Section *s)
{
  ...
  put_elf_str(s->link, "");
  section_ptr_add(s, sizeof (ObjW(Sym)));
...
ST_FUNC Section *new_symtab(CPRIMEState *s1,
                            ...
                            const char *hash_name, int hash_sh_flags)
{
  ...
  symtab->sh_entsize = sizeof(ObjW(Sym));
...
ST_FUNC void section_realloc(Section *sec, unsigned long new_size)
{
  ...
  while (size < new_size)
    size = size * 2;
  data = cprime_realloc(sec->data, size);
...
static Section *have_section(CPRIMEState *s1, const char *name)
{
  Section *sec;
  int i;
  for (i = 1; i < s1->nb_sections; i++)
...
static void rebuild_hash(Section *s, unsigned int nb_buckets)
{
  ...
  strtab = s->link->data;
  nb_syms = s->data_offset / sizeof(ObjW(Sym));
  ...
  sym = (ObjW(Sym) *)s->data + 1;
  for (sym_index = 1; sym_index < nb_syms; sym_index++)
  {
    if (Obj64_ST_BIND(sym->st_info) != STB_LOCAL)
    {
                        ...
                        int info, int other, int shndx, const char *name)
{
  ...
  sym = section_ptr_add(s, sizeof(ObjW(Sym)));
  if (name && name[0])
  ...
  if (hs)
  {
    ...
    if (Obj64_ST_BIND(info) != STB_LOCAL)
    {
...
/* return elf symbol value, signal error if 'err' is nonzero, decorate
   name if FORC */
ST_FUNC addr_t get_sym_addr(CPRIMEState *s1, const char *name, int err, int forc)
{
  int sym_index;
  ObjW(Sym) *sym;
  char buf[256];
  ...
  sym = &((ObjW(Sym) *)s1->symtab->data)[sym_index];
  if (!sym_index || sym->st_shndx == SHN_UNDEF)
  {
    if (err)
      cprime_error_noabort("%s not defined", name);
    return (addr_t) -1;
  }
  return sym->st_value;
}
...
LIBCPRIMEAPI int cprime_add_symbol(CPRIMEState *s1, const char *name, const void *val)
{
#ifdef CPRIME_TARGET_PE
                              ...
                              void (*symbol_cb)(void *ctx, const char *name, const void *val))
{
  ...
  symtab = s->symtab;
  end_sym = symtab->data_offset / sizeof (ObjSym);
  for (sym_index = 0; sym_index < end_sym; ++sym_index)
  {
    sym = &((ObjW(Sym) *)symtab->data)[sym_index];
    if (sym->st_value)
    {
      name = (char *) symtab->link->data + sym->st_name;
      sym_bind = Obj64_ST_BIND(sym->st_info);
      sym_vis = Obj64_ST_VISIBILITY(sym->st_other);
...
#ifndef ELF_OBJ_ONLY
static void
version_add (CPRIMEState *s1)
{
  int i;
  ObjW(Sym) *sym;
  ObjW(Verneed) *vn = NULL;
  Section *symtab;
  ...
  versym_section->sh_entsize = sizeof(ObjW(Half));
  versym_section->link = s1->dynsym;

  // Add Needed Symbols
  symtab = s1->dynsym;
  end_sym = symtab->data_offset / sizeof (ObjSym);
  versym = section_ptr_add(versym_section, end_sym *sizeof(ObjW(Half)));
  for (sym_index = 1; sym_index < end_sym; ++sym_index)
  {
    ...
    if (verndx >= 0
        /* XXX: on android, clang refuses to link with a libcprime.so made by cpc
        ...
        && (sym->st_shndx == SHN_UNDEF || (s1->output_type & CPRIME_OUTPUT_EXE))
       )
    {
  ...
  if (nb_versions > 2)
  {
    ...
    for (i = nb_sym_versions; i-- > 0;)
    {
      ...
      /* abitest-cpc fails on older i386-linux with "ld-linux.so.2" DT_NEEDED
      ...
      if (strcmp(sv->lib, "ld-linux.so.2"))
        cprime_add_dllref(s1, sv->lib, 0);

      vnofs = section_add(verneed_section, sizeof(*vn), 1);
      vn = (ObjW(Verneed) *)(verneed_section->data + vnofs);
      vn->vn_version = 1;
                        ...
                        int info, int other, int shndx, const char *name)
{
  ...
  sym_bind = Obj64_ST_BIND(info);
  sym_type = Obj64_ST_TYPE(info);
  sym_vis = Obj64_ST_VISIBILITY(other);
  ...
  if (sym_bind != STB_LOCAL)
  {
    ...
    if (!sym_index)
      goto do_def;
    esym = &((ObjW(Sym) *)s->data)[sym_index];
    if (esym->st_value == value && esym->st_size == size && esym->st_info == info
        && esym->st_other == other && esym->st_shndx == shndx)
      return sym_index;
    if (esym->st_shndx != SHN_UNDEF)
    {
      esym_bind = Obj64_ST_BIND(esym->st_info);
      // Propagate The Most Constraining Visibility
      // STV_DEFAULT(0)<STV_PROTECTED(3)<STV_HIDDEN(2)<STV_INTERNAL(1)
      esym_vis = Obj64_ST_VISIBILITY(esym->st_other);
...
#endif
        cprime_error_noabort("'%s' defined twice", name);
      }
    }
    else
    {
      esym->st_other = other;
do_patch:
      esym->st_info = Obj64_ST_INFO(sym_bind, sym_type);
      esym->st_shndx = shndx;
      esym->st_value = value;
      esym->st_size = size;
    }
...
do_def:
    sym_index = put_elf_sym(s, value, size,
                            Obj64_ST_INFO(sym_bind, sym_type), other,
                            shndx, name);
  }
  return sym_index;
}
                            ...
                            int type, int symbol, addr_t addend)
{
  ...
  if (!sr)
  {
    ...
    sr->sh_entsize = sizeof(ObjW_Rel);
    sr->link = symtab;
    sr->sh_info = s->sh_num;
    s->reloc = sr;
  }
  rel = section_ptr_add(sr, sizeof(ObjW_Rel));
  rel->r_offset = offset;
  rel->r_info = Obj64_R_INFO(symbol, type);
#if SHT_RELX == SHT_RELA
  rel->r_addend = addend;
#endif
  if (SHT_RELX != SHT_RELA && addend)
    cprime_error_noabort("non-zero addend on REL architecture");
}
...
ST_FUNC struct sym_attr *get_sym_attr(CPRIMEState *s1, int index, int alloc)
{
  ...
  if (index >= s1->nb_sym_attrs)
  {
    ...
    while (index >= n)
      n *= 2;
    tab = cprime_realloc(s1->sym_attrs, n *sizeof(*s1->sym_attrs));
    s1->sym_attrs = tab;
...
static void update_relocs(CPRIMEState *s1, Section *s, int *old_to_new_syms, int first_sym)
{
  ...
  for (i = 1; i < s1->nb_sections; i++)
  {
    ...
    {
      for_each_elem(sr, 0, rel, ObjW_Rel)
      {
        ...
        rel->r_info = Obj64_R_INFO(sym_index, type);
      }
    }
  }
}
...
/* In an ELF file symbol table, the local symbols must appear below
   the global and weak ones. Since CPRIME cannot sort it while generating
   the code, we must do it after. All the relocation tables are also
   modified to take into account the symbol table sorting */
static void sort_syms(CPRIMEState *s1, Section *s)
{
  int *old_to_new_syms;
  ObjW(Sym) *new_syms;
  int nb_syms, i;
  ...
  nb_syms = s->data_offset / sizeof(ObjW(Sym));
  new_syms = cprime_malloc(nb_syms *sizeof(ObjW(Sym)));
  old_to_new_syms = cprime_malloc(nb_syms *sizeof(int));
  ...
  for (i = 0; i < nb_syms; i++)
  {
    if (Obj64_ST_BIND(p->st_info) == STB_LOCAL)
    {
  ...
  memcpy(s->data, new_syms, nb_syms *sizeof(ObjW(Sym)));
  cprime_free(new_syms);

  update_relocs(s1, s, old_to_new_syms, 0);
  cprime_free(old_to_new_syms);
}
...
static Section *create_gnu_hash(CPRIMEState *s1)
{
  ...
  nb_syms = dynsym->data_offset / sizeof(ObjW(Sym));

  // Count Def Symbols
  ndef = 0;
  p = (ObjW(Sym) *)dynsym->data;
...
static Obj32_Word elf_gnu_hash (const unsigned char *name)
{
  Obj32_Word h = 5381;
  unsigned char c;
...
static void update_gnu_hash(CPRIMEState *s1, Section *gnu_hash)
{
  int *old_to_new_syms;
  ObjW(Sym) *new_syms;
  ...
  Section *vs;
  Section *dynsym = s1->dynsym;
  Obj32_Word *ptr, *buckets, *chain, *hash;
  ...
  new_syms = cprime_malloc(nb_syms *sizeof(ObjW(Sym)));
  old_to_new_syms = cprime_malloc(nb_syms *sizeof(int));
  hash = cprime_malloc(nb_syms *sizeof(Obj32_Word));
  nextbuck = cprime_malloc(nb_syms *sizeof(int));
  ...
  ptr = (Obj32_Word *) gnu_hash->data;
  nbuckets = ptr[0];
  bloom_size = ptr[2];
  bloom_shift = ptr[3];
  bloom = (addr_t *) (void *) &ptr[4];
  buckets = (Obj32_Word *) (void *) &bloom[bloom_size];
  chain = &buckets[nbuckets];
  buck = cprime_malloc(nbuckets *sizeof(*buck));

  if (gnu_hash->data_offset != 4 * 4 +
      PTR_SIZE *bloom_size +
      nbuckets * 4 +
      (nb_syms - (q - new_syms)) * 4)
    cprime_error_noabort ("gnu_hash size incorrect");
  ...
  memcpy(dynsym->data, new_syms, nb_syms *sizeof(ObjW(Sym)));
  cprime_free(new_syms);
  cprime_free(hash);
  cprime_free(buck);
  cprime_free(nextbuck);
  ...
  if (vs)
  {
    ObjW(Half) *newver, *versym = (ObjW(Half) *)vs->data;

    if (1/*versym*/)
    {
      newver = cprime_malloc(nb_syms *sizeof(*newver));
      for (i = 0; i < nb_syms; i++)
  ...
  // Rebuild Hash
  ptr = (Obj32_Word *) dynsym->hash->data;
...
ST_FUNC void relocate_syms(CPRIMEState *s1, Section *symtab, int do_resolve)
{
  ...
  for_each_elem(symtab, 1, sym, ObjW(Sym))
  {
    sh_num = sym->st_shndx;
    if (sh_num == SHN_UNDEF)
    {
      ...
      // Use ld.so to resolve symbol for us (for cpc -run)
      if (do_resolve)
      {
#if defined CPRIME_IS_NATIVE && !defined CPRIME_TARGET_PE
      ...
      if (sym_bind == STB_WEAK)
        ...
        cprime_error_noabort("undefined symbol '%s'", name);
...
static void relocate_section(CPRIMEState *s1, Section *s, Section *sr)
{
  ObjW_Rel *rel;
  ObjW(Sym) *sym;
  int type, sym_index;
  ...
  {
    sr->link = s1->dynsym;
    if (s1->output_type & CPRIME_OUTPUT_DYN)
    {
      ...
      if (r && (s->sh_flags & SHF_EXECINSTR))
        cprime_warning("%d relocations to %s", (unsigned)(r / sizeof *qrel), s->name);
#endif
    }
  }
#endif
...
#ifndef CPRIME_TARGET_MACHO
    if (s != s1->got
        || s1->static_link
        || s1->output_type == CPRIME_OUTPUT_MEMORY)
#endif
    {
...
/* count the number of dynamic relocations so that we can reserve
   their space */
static int prepare_dynamic_rel(CPRIMEState *s1, Section *sr)
{
  int count = 0;
  ...
  for_each_elem(sr, 0, rel, ObjW_Rel)
  {
    int sym_index = Obj64_R_SYM(rel->r_info);
    int type = Obj64_R_TYPE(rel->r_info);
    switch (type)
    {
#if defined(CPRIME_TARGET_I386)
    case R_386_32:
      if (!get_sym_attr(s1, sym_index, 0)->dyn_index
          && ((ObjW(Sym) * )symtab_section->data + sym_index)->st_shndx == SHN_UNDEF)
      {
        // don't fixup unresolved (weak) symbols
        rel->r_info = Obj64_R_INFO(sym_index, R_386_RELATIVE);
        break;
      }
    ...
    case R_386_PC32:
    {
      ObjW(Sym) *sym = &((ObjW(Sym) *)symtab_section->data)[sym_index];
      ...
      if (sym->st_shndx != SHN_UNDEF &&
          Obj64_ST_VISIBILITY(sym->st_other) == STV_HIDDEN)
      {
        rel->r_info = Obj64_R_INFO(sym_index, R_386_PLT32);
        break;
      }
    }
#elif defined(CPRIME_TARGET_X86_64)
    case R_X86_64_PC32:
    {
      ...
      if (sym->st_shndx != SHN_UNDEF &&
          Obj64_ST_VISIBILITY(sym->st_other) == STV_HIDDEN)
      {
        rel->r_info = Obj64_R_INFO(sym_index, R_X86_64_PLT32);
        break;
      }
    }
#elif defined(CPRIME_TARGET_ARM64)
...
#endif
      if (s1->output_type != CPRIME_OUTPUT_DLL)
        break;
      if (get_sym_attr(s1, sym_index, 0)->dyn_index)
        count++;
      break;
...
static int build_got(CPRIMEState *s1)
{
  ...
  return set_elf_sym(symtab_section, 0, 0, Obj64_ST_INFO(STB_GLOBAL, STT_OBJECT),
...
static struct sym_attr *put_got_entry(CPRIMEState *s1, int dyn_reloc_type,
                                      int sym_index)
{
  int need_plt_entry;
  const char *name;
  ...
  if (need_plt_entry)
  {
    ...
    attr->plt_sym = put_elf_sym(s1->symtab, attr->plt_offset, 0,
                                Obj64_ST_INFO(STB_GLOBAL, STT_FUNC), 0, s1->plt->sh_num, plt_name);
  }
  else
...
redo:
  for (i = 1; i < s1->nb_sections; i++)
  {
    ...
    if (s->link != symtab_section)
      continue;
    for_each_elem(s, 0, rel, ObjW_Rel)
    {
      type = Obj64_R_TYPE(rel->r_info);
      gotplt_entry = gotplt_entry_type(type);
      if (gotplt_entry == -1)
      {
        cprime_error_noabort ("Unknown relocation type for got: %d", type);
        continue;
      }
      sym_index = Obj64_R_SYM(rel->r_info);
      sym = &((ObjW(Sym) *)symtab_section->data)[sym_index];
      ...
      probably created by cprime_add_symbol, and thus on 64-bit
      ...
      if (gotplt_entry == AUTO_GOTPLT_ENTRY)
      {
        if (sym->st_shndx == SHN_UNDEF)
        {
          ...
          if (!PCRELATIVE_DLLPLT
              && (s1->output_type & CPRIME_OUTPUT_DYN))
            continue;
          /* Relocations for UNDEF symbols would normally need
             ...
             But CPRIME doesn't do that (at least for exes), so we
          ...
          if (s1->dynsym)
          {
            ...
            esym = (ObjW(Sym) *)s1->dynsym->data + dynindex;
            if (dynindex
                && (Obj64_ST_TYPE(esym->st_info) == STT_FUNC
                    || (Obj64_ST_TYPE(esym->st_info) == STT_NOTYPE
                        && Obj64_ST_TYPE(sym->st_info) == STT_FUNC)))
              goto jmp_slot;
          }
        }
        else if (sym->st_shndx == SHN_ABS)
        {
          if (sym->st_value == 0) // From Tcc_Add_Btstub()
            continue;
#ifndef CPRIME_TARGET_ARM
          if (PTR_SIZE != 8)
            continue;
#endif
          /* from cprime_add_symbol(): on 64 bit platforms these
      ...
      if ((type == R_386_PLT32 || type == R_386_PC32) &&
          ...
          (Obj64_ST_VISIBILITY(sym->st_other) != STV_DEFAULT ||
           Obj64_ST_BIND(sym->st_info) == STB_LOCAL ||
           s1->output_type & CPRIME_OUTPUT_EXE))
      {
        if (pass != 0)
          continue;
        rel->r_info = Obj64_R_INFO(sym_index, R_386_PC32);
        continue;
      }
#endif
           ...
           s1->output_type & CPRIME_OUTPUT_EXE))
      {
        if (pass != 0)
          continue;
        rel->r_info = Obj64_R_INFO(sym_index, R_X86_64_PC32);
        continue;
      }
#endif
      ...
      if (reloc_type == -1)
      {
        cprime_error_noabort ("Unknown relocation type: %d", type);
        continue;
      }
...
jmp_slot:
      ...
      if (reloc_type == R_JMP_SLOT)
        rel->r_info = Obj64_R_INFO(attr->plt_sym, type);
    }
  }
  if (++pass < 2)
    goto redo;
  ...
  if (got_sym) // Set Size
    ((ObjW(Sym) * )symtab_section->data)[got_sym].st_size = s1->got->data_offset;
}
...
ST_FUNC int set_global_sym(CPRIMEState *s1, const char *name, Section *sec, addr_t offs)
{
  ...
  return set_elf_sym(symtab_section, offs, 0,
                     Obj64_ST_INFO(name ? STB_GLOBAL : STB_LOCAL, STT_NOTYPE), 0, shn, name);
}

static void add_init_array_defines(CPRIMEState *s1, const char *section_name)
{
  Section *s;
  addr_t end_offset;
  char buf[1024];
...
static void set_local_sym(CPRIMEState *s1, const char *name, Section *s, int offset)
{
  int c = find_elf_sym(s1->symtab, name);
  if (c)
  {
    ObjW(Sym) *esym = (ObjW(Sym) *)s1->symtab->data + c;
    esym->st_info = Obj64_ST_INFO(STB_LOCAL, STT_NOTYPE);
...
static void cprime_compile_string_no_debug(CPRIMEState *s, const char *str)
{
  ...
  s->do_debug = 0;
  s->test_coverage = 0;
  cprime_compile_string(s, str);
  s->do_debug = save_do_debug;
  s->test_coverage = save_test_coverage;
}

#ifdef CONFIG_CPRIME_BACKTRACE
static void put_ptr(CPRIMEState *s1, Section *s, int offs)
{
  int c;
...
ST_FUNC void cprime_add_btstub(CPRIMEState *s1)
{
  ...
  if (s1->output_type == CPRIME_OUTPUT_MEMORY && 0 == s1->dwarf)
  ...
  else
  {
    ...
    if (s1->dwarf == 0 && s1->output_type == CPRIME_OUTPUT_EXE)
  ...
  cstr_printf(&cstr, "__bt_init(__rt_info,%d);}",
              s1->output_type != CPRIME_OUTPUT_DLL);
  // In case dlcose is called by application
  cstr_printf(&cstr,
              "__attribute__((destructor)) static void __bt_exit_rt(){"
              "__bt_exit(__rt_info);}");
  cprime_compile_string_no_debug(s1, cstr.data);
  cstr_free(&cstr);
  set_local_sym(s1, __rt_info, s, o);
}
#endif // def CONFIG_CPRIME_BACKTRACE 

static void cprime_tcov_add_file(CPRIMEState *s1, const char *filename)
{
  CString cstr;
  void *ptr;
  char wd[1024];
...
#if TARGETOS_OpenBSD
  if (s1->output_type != CPRIME_OUTPUT_DLL)
    cprime_add_crt(s1, "crt0.o");
  if (s1->output_type == CPRIME_OUTPUT_DLL)
    cprime_add_crt(s1, "crtbeginS.o");
  else
    cprime_add_crt(s1, "crtbegin.o");
#elif TARGETOS_FreeBSD || TARGETOS_NetBSD
  if (s1->output_type != CPRIME_OUTPUT_DLL)
#if TARGETOS_FreeBSD
    cprime_add_crt(s1, "crt1.o");
#else
    cprime_add_crt(s1, "crt0.o");
#endif
  cprime_add_crt(s1, "crti.o");
  if (s1->static_link)
    cprime_add_crt(s1, "crtbeginT.o");
  else if (s1->output_type == CPRIME_OUTPUT_DLL)
    cprime_add_crt(s1, "crtbeginS.o");
  else
    cprime_add_crt(s1, "crtbegin.o");
#elif TARGETOS_ANDROID
  if (s1->output_type == CPRIME_OUTPUT_DLL)
    cprime_add_crt(s1, "crtbegin_so.o");
  else
    cprime_add_crt(s1, "crtbegin_dynamic.o");
#else
  if (s1->output_type != CPRIME_OUTPUT_DLL)
    cprime_add_crt(s1, "crt1.o");
  cprime_add_crt(s1, "crti.o");
...
#if TARGETOS_OpenBSD
  if (s1->output_type == CPRIME_OUTPUT_DLL)
    cprime_add_crt(s1, "crtendS.o");
  else
    cprime_add_crt(s1, "crtend.o");
#elif TARGETOS_FreeBSD || TARGETOS_NetBSD
  if (s1->output_type == CPRIME_OUTPUT_DLL)
    cprime_add_crt(s1, "crtendS.o");
  else
    cprime_add_crt(s1, "crtend.o");
  cprime_add_crt(s1, "crtn.o");
#elif TARGETOS_ANDROID
  if (s1->output_type == CPRIME_OUTPUT_DLL)
    cprime_add_crt(s1, "crtend_so.o");
  else
    cprime_add_crt(s1, "crtend_android.o");
#else
  cprime_add_crt(s1, "crtn.o");
#endif
}
#endif // CPRIME_TARGET_UNIX 

#ifndef CPRIME_TARGET_PE
// Add Cpc Runtime Libraries
...
#ifdef CONFIG_CPRIME_BCHECK
    if (s1->do_bounds_check && s1->output_type != CPRIME_OUTPUT_DLL)
    {
      cprime_add_support(s1, "bcheck.o");
# if !(TARGETOS_OpenBSD || TARGETOS_NetBSD)
      cprime_add_library(s1, "dl");
# endif
      lpthread = 1;
    }
#endif
    ...
    if (s1->do_backtrace)
    {
      if (s1->output_type & CPRIME_OUTPUT_EXE)
        cprime_add_support(s1, "bt-exe.o");
      if (s1->output_type != CPRIME_OUTPUT_DLL)
        cprime_add_support(s1, "bt-log.o");
      cprime_add_btstub(s1);
      lpthread = 1;
    }
#endif
    if (lpthread)
      cprime_add_library(s1, "pthread");
    cprime_add_library(s1, "c");
#ifdef CPRIME_LIBGCC
    if (!s1->static_link)
    {
      if (CPRIME_LIBGCC[0] == '/')
        cprime_add_file(s1, CPRIME_LIBGCC);
      else
        cprime_add_dll(s1, CPRIME_LIBGCC, AFF_PRINT_ERROR);
    }
#endif
#if defined CPRIME_TARGET_ARM && TARGETOS_FreeBSD
    cprime_add_library(s1, "gcc_s"); // Unwind Code
#endif
    if (CPRIME_LIBCPRIME1[0])
      cprime_add_support(s1, CPRIME_LIBCPRIME1);
#ifndef CPRIME_TARGET_MACHO
    if (s1->output_type != CPRIME_OUTPUT_MEMORY)
      cprimeelf_add_crtend(s1);
#endif
  }
}
#endif // ndef CPRIME_TARGET_PE 

/* add various standard linker symbols (must be done after the
   sections are filled (for example after allocating common
   symbols)) */
static void cprime_add_linker_symbols(CPRIMEState *s1)
{
  char buf[1024];
  int i;
  Section *s;
...
ST_FUNC void resolve_common_syms(CPRIMEState *s1)
{
  ...
  cprime_add_linker_symbols(s1);
}

#ifndef ELF_OBJ_ONLY
ST_FUNC void fill_got_entry(CPRIMEState *s1, ObjW_Rel *rel)
{
  int sym_index = Obj64_R_SYM (rel->r_info);
  ObjW(Sym) *sym = &((ObjW(Sym) *) symtab_section->data)[sym_index];
...
ST_FUNC void fill_got(CPRIMEState *s1)
{
  Section *s;
  ObjW_Rel *rel;
  int i;
  ...
  for (i = 1; i < s1->nb_sections; i++)
  {
    ...
    for_each_elem(s, 0, rel, ObjW_Rel)
    {
      switch (Obj64_R_TYPE (rel->r_info))
      {
      case R_X86_64_GOT32:
...
static void fill_local_got_entries(CPRIMEState *s1)
{
  ObjW_Rel *rel;
  if (!s1->got->reloc)
    return;
  for_each_elem(s1->got->reloc, 0, rel, ObjW_Rel)
  {
    if (Obj64_R_TYPE(rel->r_info) == R_RELATIVE)
    {
      int sym_index = Obj64_R_SYM (rel->r_info);
      ObjW(Sym) *sym = &((ObjW(Sym) *) symtab_section->data)[sym_index];
      struct sym_attr *attr = get_sym_attr(s1, sym_index, 0);
      ...
      if (offset != rel->r_offset - s1->got->sh_addr)
        cprime_error_noabort("fill_local_got_entries: huh?");
      rel->r_info = Obj64_R_INFO(0, R_RELATIVE);
#if SHT_RELX == SHT_RELA
...
/* Bind symbols of executable: resolve undefined symbols from exported symbols
   in shared libraries */
static void bind_exe_dynsyms(CPRIMEState *s1, int is_PIE)
{
  const char *name;
  int sym_index, index;
  ObjW(Sym) *sym, *esym;
  ...
  for_each_elem(symtab_section, 1, sym, ObjW(Sym))
  {
    if (sym->st_shndx == SHN_UNDEF)
    {
      ...
      if (sym_index)
      {
        if (is_PIE)
          continue;
        esym = &((ObjW(Sym) *)s1->dynsymtab_section->data)[sym_index];
        type = Obj64_ST_TYPE(esym->st_info);
        if ((type == STT_FUNC) || (type == STT_GNU_IFUNC))
        {
          ...
          int dynindex
            = put_elf_sym(s1->dynsym, 0, esym->st_size,
                          Obj64_ST_INFO(STB_GLOBAL, STT_FUNC), 0, 0,
                          name);
          int index = sym - (ObjW(Sym) *) symtab_section->data;
        ...
        else if (type == STT_OBJECT)
        {
          ...
          if (Obj64_ST_BIND(esym->st_info) == STB_WEAK)
          {
            for_each_elem(s1->dynsymtab_section, 1, dynsym, ObjW(Sym))
            {
              if ((dynsym->st_value == esym->st_value)
                  && (Obj64_ST_BIND(dynsym->st_info) == STB_GLOBAL))
              {
      ...
      else
      {
        ...
        if (Obj64_ST_BIND(sym->st_info) == STB_WEAK ||
            !strcmp(name, "_fp_hw"))
        {
        }
        else
...
static void bind_libs_dynsyms(CPRIMEState *s1)
{
  const char *name;
  int dynsym_index;
  ObjW(Sym) *sym, *esym;
  ...
  for_each_elem(symtab_section, 1, sym, ObjW(Sym))
  {
    ...
    {
      esym = (ObjW(Sym) *)s1->dynsymtab_section->data + dynsym_index;
      if (esym->st_shndx == SHN_UNDEF)
      {
        // Weak Symbols Can Stay Undefined
        if (Obj64_ST_BIND(esym->st_info) != STB_WEAK)
          cprime_warning("undefined dynamic symbol '%s'", name);
      }
    }
  }
}
...
static void export_global_syms(CPRIMEState *s1)
{
  int dynindex, index;
  const char *name;
  ObjW(Sym) *sym;
  for_each_elem(symtab_section, 1, sym, ObjW(Sym))
  {
    if (Obj64_ST_BIND(sym->st_info) != STB_LOCAL)
    {
      ...
      dynindex = set_elf_sym(s1->dynsym, sym->st_value, sym->st_size,
                             sym->st_info, 0, sym->st_shndx, name);
      index = sym - (ObjW(Sym) *) symtab_section->data;
      get_sym_attr(s1, index, 1)->dyn_index = dynindex;
    }
  }
}
...
static int set_sec_sizes(CPRIMEState *s1)
{
  int i;
  Section *s;
  int textrel = 0;
  ...
  for (i = 1; i < s1->nb_sections; i++)
  {
    ...
    if (s->sh_type == SHT_RELX && !(s->sh_flags & SHF_ALLOC))
    {
      /* when generating a DLL, we include relocations but
         we may patch them */
      if ((file_type & CPRIME_OUTPUT_DYN)
          && (s1->sections[s->sh_info]->sh_flags & SHF_ALLOC))
      {
        ...
        if (count)
        {
          ...
          s->sh_size = count * sizeof(ObjW_Rel);
...
/* Decide the layout of sections loaded in memory. This must be done before
   ...
   nobits */
static int sort_sections(CPRIMEState *s1, int *sec_order, struct dyn_inf *d)
{
  Section *s;
  int i, j, k, f, f0, n;
  ...
  for (i = 1; i < nb_sections; i++)
  {
    ...
    else if (s->sh_flags & SHF_ALLOC)
    {
      ...
      j = 0x700;
    if (j >= 0x700 && s1->output_format != CPRIME_OUTPUT_FORMAT_ELF)
      s->sh_size = 0, j = 0x900;
...
static ObjW(Phdr) *fill_phdr(ObjW(Phdr) *ph, int type, Section *s)
{
  if (s)
  {
...
static int layout_sections(CPRIMEState *s1, int *sec_order, struct dyn_inf *d)
{
  Section *s;
  ...
  if (d->roinf)
    ++phnum;
  d->phnum = phnum;
  d->phdr = cprime_mallocz(phnum *sizeof(ObjW(Phdr)));

  file_offset = 0;
  if (s1->output_format == CPRIME_OUTPUT_FORMAT_ELF)
  {
    file_offset = (sizeof(ObjW(Ehdr)) + phnum *sizeof(ObjW(Phdr)) + 3) & -4;
    file_offset += d->shnum * sizeof (ObjW(Shdr));
  }
  ...
  {
    ph = &d->phdr[0];
    ph->p_offset = sizeof(ObjW(Ehdr));
    ph->p_vaddr = base + ph->p_offset;
    ph->p_filesz = phnum * sizeof(ObjW(Phdr));
    ph->p_align = 4;
...
{
  ObjW(Dyn) *dyn;
  dyn = section_ptr_add(dynamic, sizeof(ObjW(Dyn)));
  dyn->d_tag = dt;
  dyn->d_un.d_val = val;
}
...
/* Fill the dynamic section with tags describing the address and size of
   sections */
static void fill_dynamic(CPRIMEState *s1, struct dyn_inf *dyninf)
{
  ...
  put_dt(dynamic, DT_SYMENT, sizeof(ObjW(Sym)));
#if PTR_SIZE == 8
  put_dt(dynamic, DT_RELA, dyninf->rel_addr);
  put_dt(dynamic, DT_RELASZ, dyninf->rel_size);
  put_dt(dynamic, DT_RELAENT, sizeof(ObjW_Rel));
  if (s1->plt && s1->plt->reloc)
  {
  ...
  put_dt(dynamic, DT_RELENT, sizeof(ObjW_Rel));
  if (s1->plt && s1->plt->reloc)
  {
    put_dt(dynamic, DT_PLTGOT, s1->got->sh_addr);
    put_dt(dynamic, DT_PLTRELSZ, s1->plt->reloc->data_offset);
...
/* Remove gaps between RELX sections.
   ...
   The gaps are then filled with 0 in cprime_output_elf. The 0 is intepreted as
...
static void update_reloc_sections(CPRIMEState *s1, struct dyn_inf *dyninf)
{
  int i;
...
static int cprime_output_elf(CPRIMEState *s1, FILE *f, int phnum, ObjW(Phdr) *phdr)
{
  ...
  if (phnum > 0)
  {
    ehdr.e_phentsize = sizeof(ObjW(Phdr));
    ehdr.e_phnum = phnum;
    ehdr.e_phoff = sizeof(ObjW(Ehdr));
  }
...
#elif defined CPRIME_TARGET_ARM && defined CPRIME_ARM_EABI
  ...
  if (file_type == CPRIME_OUTPUT_OBJ)
    ehdr.e_type = ET_REL;
  else
  {
    if (file_type & CPRIME_OUTPUT_DYN)
    ...
    if (s1->elf_entryname)
      ...
      ehdr.e_entry = get_sym_addr(s1, "_start", !!(file_type & CPRIME_OUTPUT_EXE), 0);
  ...
  ehdr.e_shoff = (sizeof(ObjW(Ehdr)) + phnum *sizeof(ObjW(Phdr)) + 3) & -4;
  ehdr.e_ehsize = sizeof(ObjW(Ehdr));
  ehdr.e_shentsize = sizeof(ObjW(Shdr));
  ehdr.e_shnum = shnum;
  ehdr.e_shstrndx = shnum - 1;

  offset = fwrite(&ehdr, 1, sizeof(ObjW(Ehdr)), f);
  if (phdr)
    offset += fwrite(phdr, 1, phnum *sizeof(ObjW(Phdr)), f);
  ...
  for (i = 0; i < shnum; i++)
  {
    ...
    offset += fwrite(sh, 1, sizeof(ObjW(Shdr)), f);
  }
...
static int cprime_output_binary(CPRIMEState *s1, FILE *f)
{
  Section *s;
  int i, offset, size;
...
static int cprime_write_elf_file(CPRIMEState *s1, const char *filename, int phnum,
                              ObjW(Phdr) *phdr)
{
  ...
  if (fd < 0 || (f = fdopen(fd, "wb")) == NULL)
    return cprime_error_noabort("could not write '%s: %s'", filename, strerror(errno));
  if (s1->verbose)
    printf("<- %s\n", filename);
#ifdef CPRIME_TARGET_COFF
  if (s1->output_format == CPRIME_OUTPUT_FORMAT_COFF)
    cprime_output_coff(s1, f);
  else
#endif
    if (s1->output_format == CPRIME_OUTPUT_FORMAT_ELF)
      ret = cprime_output_elf(s1, f, phnum, phdr);
    else
      ret = cprime_output_binary(s1, f);
  fclose(f);
...
static void reorder_sections(CPRIMEState *s1, int *sec_order)
{
  int i, nnew, k, *backmap;
  Section **snew, *s;
  ObjW(Sym) *sym;

  backmap = cprime_malloc(s1->nb_sections *sizeof(backmap[0]));
  ...
  for (i = 1; i < nnew; i++)
  {
    ...
    if (s->sh_type == SHT_RELX)
      ...
      for_each_elem(s, 1, sym, ObjW(Sym))
      if (sym->st_shndx < s1->nb_sections)
        sym->st_shndx = backmap[sym->st_shndx];
  }
  cprime_free(s1->sections);
  s1->sections = snew;
  s1->nb_sections = nnew;
  cprime_free(backmap);
}
...
#ifdef CPRIME_TARGET_ARM
static void create_arm_attribute_section(CPRIMEState *s1)
{
                          ...
                          const char *value, uint32_t data)
{
  ...
  while (offset + sizeof(ObjW(Nhdr)) < s->data_offset)
  {
    note = (ObjW(Nhdr) *) (s->data + offset);
    if (note->n_type == type)
      return;
    offset += (sizeof(ObjW(Nhdr)) + note->n_namesz + note->n_descsz +
               align - 1) & -align;
  }
  ptr = section_ptr_add(s, sizeof(ObjW(Nhdr)) + 8 + 4);
  note = (ObjW(Nhdr) *) ptr;
  note->n_namesz = 8;
  note->n_descsz = 4;
  note->n_type = type;
  strcpy (ptr + sizeof(ObjW(Nhdr)), value);
  memcpy (ptr + sizeof(ObjW(Nhdr)) + 8, &data, 4);
}

static Section *create_bsd_note_section(CPRIMEState *s1,
                                        const char *name,
                                        const char *value)
{
  Section *s;
...
static void alloc_sec_names(CPRIMEState *s1, int is_obj);

// Output an elf, coff or binary file
// XXX: suppress unneeded sections
static int elf_output_file(CPRIMEState *s1, const char *filename)
{
  ...
  cprime_add_runtime(s1);
  resolve_common_syms(s1);

  if (!s1->static_link)
  {
    if (file_type & CPRIME_OUTPUT_EXE)
    {
      char *ptr;
    ...
    dynamic = new_section(s1, ".dynamic", SHT_DYNAMIC,
                          SHF_ALLOC | SHF_WRITE);
    dynamic->link = dynstr;
    dynamic->sh_entsize = sizeof(ObjW(Dyn));

    got_sym = build_got(s1);
    if (file_type & CPRIME_OUTPUT_EXE)
    {
      bind_exe_dynsyms(s1, file_type &CPRIME_OUTPUT_DYN);
      if (s1->nb_errors)
        goto the_end;
    }
  ...
  sec_order = cprime_malloc(sizeof(int) * 2 * s1->nb_sections);
  ...
  if (dynamic)
  {
    ...
    if (file_type == CPRIME_OUTPUT_EXE
        || (RELOCATE_DLLPLT && (file_type & CPRIME_OUTPUT_DYN)))
      relocate_plt(s1);
  ...
  // Perform relocation to GOT or PLT entries
  if (file_type == CPRIME_OUTPUT_EXE && s1->static_link)
    fill_got(s1);
  else if (s1->got)
  ...
  cprime_eh_frame_hdr(s1, 1);
#endif
  // Create the ELF file with name 'filename'
  ret = cprime_write_elf_file(s1, filename, dyninf.phnum, dyninf.phdr);
the_end:
  cprime_free(sec_order);
  cprime_free(dyninf.phdr);
  return ret;
}
#endif // ndef ELF_OBJ_ONLY 
...
LIBCPRIMEAPI int elf_output_obj(CPRIMEState *s1, const char *filename)
{
  ...
  file_offset = (sizeof (ObjW(Ehdr)) + 3) & -4;
  file_offset += s1->nb_sections * sizeof(ObjW(Shdr));
  for (i = 1; i < s1->nb_sections; i++)
  {
    s = s1->sections[i];
  ...
  // Create the ELF file with name 'filename'
  ret = cprime_write_elf_file(s1, filename, 0, NULL);
  return ret;
}
...
ST_FUNC int cprime_load_object_file(CPRIMEState *s1,
                                 int fd, unsigned long file_offset)
{
  ...
  {
invalid:
    return cprime_error_noabort("invalid object file");
  }
  // Read Sections
  shdr = load_data(fd, file_offset + ehdr.e_shoff,
                   sizeof(ObjW(Shdr)) * ehdr.e_shnum);
  sm_table = cprime_mallocz(sizeof(SectionMergeInfo) * ehdr.e_shnum);
  ...
  for (i = 1; i < ehdr.e_shnum; i++)
  {
    ...
    if (sh->sh_type == SHT_SYMTAB)
    {
      if (symtab)
      {
        cprime_error_noabort("object must contain only one symtab");
        goto the_end;
      }
      nb_syms = sh->sh_size / sizeof(ObjW(Sym));
  ...
  for (i = 1; i < ehdr.e_shnum; i++)
  {
    ...
    for (j = 1; j < s1->nb_sections; j++)
    {
      ...
      {
        cprime_error_noabort("section type conflict: %s %02x <> %02x", s->name, sh->sh_type, s->sh_type);
        goto the_end;
      }
...
#if defined CPRIME_TARGET_ARM || defined CPRIME_TARGET_ARM64 || defined CPRIME_TARGET_RISCV64
...
next: ;
  ...
  // Resolve Symbols
  old_to_new_syms = cprime_mallocz(nb_syms *sizeof(int));
  ...
  for (i = 1; i < ehdr.e_shnum; i++)
  {
    ...
    case SHT_RELX:
      ...
      for (rel = (ObjW_Rel *) s->data + (offset / sizeof(*rel));
           rel < (ObjW_Rel *) s->data + ((offset + size) / sizeof(*rel));
           rel++)
      {
        int type;
        unsigned sym_index;
        ...
        {
invalid_reloc:
          cprime_error_noabort("Invalid relocation entry [%2d] '%s' @ %.8x",
...
static int cprime_load_alacarte(CPRIMEState *s1, int fd, int size, int entrysize)
{
  ...
  do
  {
    ...
    for (p = ar_names, i = 0; i < nsyms; i++, p += strlen(p) + 1)
    {
      ...
      {
invalid:
        cprime_error_noabort("invalid archive");
        goto the_end;
      }
      off += len;
      if (s1->verbose == 2)
...
ST_FUNC int cprime_load_archive(CPRIMEState *s1, int fd, int alacarte)
{
  ...
  for (;;)
  {
    ...
    if (len == 0)
      return 0;
    if (len < 0)
      return cprime_error_noabort("invalid archive");
    file_offset += len;
    ...
    if (alacarte)
    {
      ...
      if (!strcmp(hdr.ar_name, "/"))
        return cprime_load_alacarte(s1, fd, size, 4);
    ...
    else if (cprime_object_type(fd, &ehdr) == AFF_BINTYPE_REL)
    {
      if (s1->verbose == 2)
        printf("   -> %s\n", hdr.ar_name);
      if (cprime_load_object_file(s1, fd, file_offset) < 0)
        return -1;
    }
    // Align To Even
...
static void set_ver_to_ver(CPRIMEState *s1, int *n, int **lv, int i, char *lib, char *version)
{
  while (i >= *n)
  {
    *lv = cprime_realloc(*lv, (*n + 1) * sizeof(**lv));
    (*lv)[(*n)++] = -1;
  }
  if ((*lv)[i] == -1)
  {
    ...
    {
      sym_versions = cprime_realloc (sym_versions,
                                  (v + 1) * sizeof(*sym_versions));
      sym_versions[v].lib = cprime_strdup(lib);
      sym_versions[v].version = cprime_strdup(version);
...
/* Associates symbol SYM_INDEX (in dynsymtab) with sym-version index
   VERNDX.  */
static void
set_sym_version(CPRIMEState *s1, int sym_index, int verndx)
{
  if (sym_index >= nb_sym_to_version)
  {
    int newelems = sym_index ? sym_index * 2 : 1;
    sym_to_version = cprime_realloc(sym_to_version,
...
struct versym_info
{
  int nb_versyms;
  ObjW(Verdef) *verdef;
  ObjW(Verneed) *verneed;
  ObjW(Half) *versym;
  int nb_local_ver, *local_ver;
};


static void store_version(CPRIMEState *s1, struct versym_info *v, char *dynstr)
{
  char *lib, *version;
  uint32_t next;
  int i;
...
#define DEBUG_VERSION 0
  ...
  {
    ObjW(Verdef) *vdef = v->verdef;
    lib = NULL;
    do
    {
      ObjW(Verdaux) *verdaux =
        (ObjW(Verdaux) *) (((char *) vdef) + vdef->vd_aux);
      ...
      next = vdef->vd_next;
      vdef = (ObjW(Verdef) *) (((char *) vdef) + next);
    }
    while (next);
  }
  if (v->versym && v->verneed)
  {
    ObjW(Verneed) *vneed = v->verneed;
    do
    {
      ObjW(Vernaux) *vernaux =
        (ObjW(Vernaux) *) (((char *) vneed) + vneed->vn_aux);
      ...
      for (i = 0; i < vneed->vn_cnt; i++)
      {
        ...
        vernaux = (ObjW(Vernaux) *) (((char *) vernaux) + vernaux->vna_next);
      }
      next = vneed->vn_next;
      vneed = (ObjW(Verneed) *) (((char *) vneed) + next);
    }
    while (next);
  }
...
ST_FUNC int cprime_load_dll(CPRIMEState *s1, int fd, const char *filename, int level)
{
  ...
  if (ehdr.e_ident[5] != ELFDATA2LSB ||
      ehdr.e_machine != EM_CPRIME_TARGET)
    return cprime_error_noabort("bad architecture");

  // Read Sections
  shdr = load_data(fd, ehdr.e_shoff, sizeof(ObjW(Shdr)) * ehdr.e_shnum);
  ...
  for (i = 0, sh = shdr; i < ehdr.e_shnum; i++, sh++)
  {
    ...
    {
    case SHT_DYNAMIC:
      nb_dts = sh->sh_size / sizeof(ObjW(Dyn));
    ...
    case SHT_GNU_verneed:
      ...
      break;
    case SHT_GNU_versym:
      v.nb_versyms = sh->sh_size / sizeof(ObjW(Half));
  ...
  soname = cprime_basename(filename);
  for (i = 0, dt = dynamic; i < nb_dts; i++, dt++)
    if (dt->d_tag == DT_SONAME)
      soname = dynstr + dt->d_un.d_val;

  // if the dll is already loaded, do not load it
  if (cprime_add_dllref(s1, soname, level)->found)
    goto ret_success;

  if (v.nb_versyms != nb_syms)
    cprime_free (v.versym), v.versym = NULL;
  else
  ...
  /* following DT_NEEDED is needed for the dynamic loader (libdl.so),
     ...
     When cpc output mode is OUTPUT_MEM,
     cpc calls dlopen, which handles DT_NEEDED for us */

#if 0
  for (i = 0, dt = dynamic; i < nb_dts; i++, dt++)
    if (dt->d_tag == DT_RPATH)
      cprime_add_library_path(s1, dynstr + dt->d_un.d_val);
  ...
  for (i = 0, dt = dynamic; i < nb_dts; i++, dt++)
  {
    ...
    case DT_NEEDED:
      ...
      if (cprime_add_dllref(s1, name, -1))
        continue;
      if (cprime_add_dll(s1, name, AFF_REFERENCED_DLL) < 0)
      {
        ret = cprime_error_noabort("referenced dll '%s' not found", name);
        goto the_end;
      }
    }
  }
...
the_end:
  ...
  cprime_free(dynamic);
  cprime_free(shdr);
  cprime_free(v.local_ver);
  cprime_free(v.verdef);
  cprime_free(v.verneed);
  cprime_free(v.versym);
  return ret;
}

#define LD_TOK_NAME 256
#define LD_TOK_EOF  (-1)
static int ld_inp(CPRIMEState *s1)
{
  int c = *s1->ld_p;
  if (c == 0)
    return CH_EOF;
...
static int ld_next(CPRIMEState *s1, char *name, int name_size)
{
  int c, d, ch;
  char *q;
...
static int ld_add_file(CPRIMEState *s1, const char filename[])
{
  if (filename[0] == '-' && filename[1] == 'l')
    return cprime_add_library(s1, filename + 2);
  if (CONFIG_SYSROOT[0] != '\0' || !IS_ABSPATH(filename))
  {
    // Lookup Via Library Paths
    int ret = cprime_add_dll(s1, cprime_basename(filename), 0);
    if (ret != FILE_NOT_FOUND)
      return ret;
  }
  return cprime_add_file_internal(s1, filename, AFF_PRINT_ERROR);
}

// Did Static Libraries Add New Undefined Symbols?
static int new_undef_sym(CPRIMEState *s1, int sym_offset)
{
  while (sym_offset < s1->symtab->data_offset)
  {
    ObjW(Sym) *esym = (void *)(s1->symtab->data + sym_offset);
    if (esym->st_shndx == SHN_UNDEF)
      return 1;
    sym_offset += sizeof (ObjW(Sym));
  }
  return 0;
}

static int ld_add_file_list(CPRIMEState *s1, const char *cmd)
{
  char filename[1024];
...
repeat:
  ...
  if (t != '(')
    return cprime_error_noabort("expected '(' after %s", cmd);
  t = ld_next(s1, filename, sizeof(filename));
  for (;;)
  {
    if (t == LD_TOK_EOF)
      return cprime_error_noabort("unexpected end of file");
    else if (t == ')')
      break;
    else if (t != LD_TOK_NAME)
      return cprime_error_noabort("unexpected token '%c'", t);
...
ST_FUNC int cprime_load_ldscript(CPRIMEState *s1, int fd)
{
  ...
  saved_ptr = s1->ld_p;
  s1->ld_p = text_ptr = (void *)cprime_load_text(fd);
  for (;;)
  {
             ...
             !strcmp(cmd, "TARGET"))
    {
      ...
      ret = cprime_error_noabort("unexpected '%s'", cmd);
    if (ret < 0)
      break;
    noscript = 0;
  }
  cprime_free(text_ptr);
  s1->ld_p = saved_ptr;
