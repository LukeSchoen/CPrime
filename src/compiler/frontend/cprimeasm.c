...
ST_FUNC int asm_get_local_label_name(CPRIMEState *s1, unsigned int n)
{
  return asm_get_prefix_name(s1, "L..", n);
}

static int cprime_assemble_internal(CPRIMEState *s1, int do_preprocess, int global);
static Sym *asm_new_label(CPRIMEState *s1, int label, int is_local);
static Sym *asm_new_label1(CPRIMEState *s1, int label, int is_local, int sh_num, int value);
...
static int asm2cname(int v, int *addeddot)
{
  const char *name;
  *addeddot = 0;
  if (!cprime_state->leading_underscore)
    return v;
...
static Sym *asm_section_sym(CPRIMEState *s1, Section *sec)
{
...
static void asm_expr_unary(CPRIMEState *s1, ExprValue *pe)
{
  Sym *sym;
  int op, label;
  uint64_t n;
  ...
  case TOK_PPNUM:
    ...
    if (*p == 'b' || *p == 'f')
    {
      ...
      if (*p == 'b')
      {
        ...
        if (sym && (!sym->c || elfsym(sym)->st_shndx == SHN_UNDEF))
          sym = sym->prev_tok;
        if (!sym)
          cprime_error("local label '%d' not found backward", (int)n);
      }
      else
      {
        // Forward
    ...
    else if (*p == '\0')
    {
      ...
      cprime_error("invalid number syntax");
    next();
    break;
  case '+':
    next();
  ...
  case '~':
    ...
    asm_expr_unary(s1, pe);
    if (pe->sym)
      cprime_error("invalid operation with label");
    if (op == '-')
      pe->v = -pe->v;
    else
      pe->v = ~pe->v;
  ...
  default:
    if (tok >= TOK_IDENT)
    {
      ...
      cprime_error("bad expression syntax [%s]", get_tok_str(tok, &tokc));
    break;
  }
}

static void asm_expr_prod(CPRIMEState *s1, ExprValue *pe)
{
  int op;
  ExprValue e2;
...
static void asm_expr_logic(CPRIMEState *s1, ExprValue *pe)
{
  int op;
  ExprValue e2;
...
static inline void asm_expr_sum(CPRIMEState *s1, ExprValue *pe)
{
  int op;
  ExprValue e2;
...
static inline void asm_expr_cmp(CPRIMEState *s1, ExprValue *pe)
{
  int op;
  ExprValue e2;
...
ST_FUNC int asm_int_expr(CPRIMEState *s1)
{
  ...
  if (e.sym)
    expect("constant");
  if ((int)e.v != e.v)
    cprime_error("integer out of range %lld", (long long)e.v);
  return e.v;
}

static Sym *asm_new_label1(CPRIMEState *s1, int label, int is_local,
  ...
  if (sym)
  {
    ...
    if (esym && esym->st_shndx != SHN_UNDEF)
    {
      ...
      if (!(sym->type.t & VT_EXTERN))
        cprime_error("assembler label '%s' already defined",
...
static Sym *set_symbol(CPRIMEState *s1, int label)
{
  long n;
  ExprValue e;
  Sym *sym;
...
static void use_section1(CPRIMEState *s1, Section *sec)
{
...
static void use_section(CPRIMEState *s1, const char *name)
{
  Section *sec;
...
static void push_section(CPRIMEState *s1, const char *name)
{
...
static void pop_section(CPRIMEState *s1)
{
  Section *prev = cur_text_section->prev;
  if (!prev)
    cprime_error(".popsection without .pushsection");
  cur_text_section->prev = NULL;
  use_section1(s1, prev);
}

static void asm_parse_directive(CPRIMEState *s1, int global)
{
  ...
  case TOK_ASMDIR_space:
    ...
    {
      if (n < 0 || n > 30)
        cprime_error("invalid p2align, must be between 0 and 30");
      n = 1 << n;
...
asm_data:
  ...
  case TOK_ASMDIR_fill:
  {
    ...
    if (repeat < 0)
    {
      cprime_error("repeat < 0; .fill ignored");
      break;
    }
    size = 1;
    val = 0;
    if (tok == ',')
    {
      ...
      if (size < 0)
      {
        cprime_error("size < 0; .fill ignored");
        break;
      }
      if (size > 8)
        size = 8;
  ...
  case TOK_ASMDIR_rept:
  {
    ...
    {
      if (tok == CH_EOF)
        cprime_error("we at end of file, .endr not found");
    ...
    while (repeat-- > 0)
    {
      cprime_assemble_internal(s1, (parse_flags &PARSE_FLAG_PREPROCESS),
                            global);
  ...
  case TOK_ASMDIR_org:
  {
    ...
    if (n < 0 || n > 0x100000)
      cprime_error(".org out of range");
    if (n < ind)
      cprime_error("attempt to .org backwards");
    v = 0;
    size = n - ind;
    goto zero_pad;
  }
  ...
  case TOK_ASMDIR_ident:
  {
    ...
    cprime_warning_c(warn_unsupported)("ignoring .ident %s", ident);
    next();
  }
  break;
  case TOK_ASMDIR_size:
  {
    ...
    if (!sym)
      cprime_error("label not found: %s", get_tok_str(tok, NULL));
    // XXX .size name,label2-label1
    cprime_warning_c(warn_unsupported)("ignoring .size %s,*", get_tok_str(tok, NULL));
    next();
    skip(',');
...
set_st_type:
      if (sym->c)
      {
        ...
        esym->st_info = Obj64_ST_INFO(Obj64_ST_BIND(esym->st_info), st_type);
      }
    }
    else if (!strcmp(newtype, "object") || !strcmp(newtype, "STT_OBJECT"))
    {
      ...
      cprime_warning_c(warn_unsupported)("change type of '%s' from 0x%x to '%s' ignored",
  ...
  case TOK_ASMDIR_previous:
  {
    ...
    if (!last_text_section)
      cprime_error("no previous section referenced");
    sec = cur_text_section;
  ...
  case TOK_ASMDIR_option:
    ...
    case TOK_ASM_push:
    case TOK_ASM_pop:
      // Reserved for future .option support.
      next();
      break;
    case TOK_ASM_arch:
      // This form still needs extra parsing support.
      cprime_error("unimp .option '.%s'", get_tok_str(tok, NULL));
      break;
    default:
      cprime_error("unknown .option '.%s'", get_tok_str(tok, NULL));
      break;
    }
    break;
#endif
  // symver parsing is still a separate feature gap; FreeBSD >= 14 needs it.
  ...
  break;
  default:
    cprime_error("unknown assembler directive '.%s'", get_tok_str(tok, NULL));
    break;
  }
}
...
ST_FUNC int cprime_assemble(CPRIMEState *s1, int do_preprocess)
{
  ...
  ind = cur_text_section->data_offset;
  nocode_wanted = 0;
  ret = cprime_assemble_internal(s1, do_preprocess, 1);
  cur_text_section->data_offset = ind;
  cprime_debug_end(s1);
  return ret;
}
...
/* assemble the string 'str' in the current C compilation unit without
   C preprocessing. */
static void cprime_assemble_inline(CPRIMEState *s1, const char *str, int len, int global)
{
  const int *saved_macro_ptr = macro_ptr;
  int dotid = set_idnum('.', IS_ID);
#ifndef CPRIME_TARGET_RISCV64
  int dolid = set_idnum('$', 0);
#endif

  cprime_open_bf(s1, ":asm:", len);
  memcpy(file->buffer, str, len);
  macro_ptr = NULL;
  cprime_assemble_internal(s1, 0, global);
  cprime_close();
  ...
  if (tok != ':')
  {
    ...
    for (;;)
    {
      if (nb_operands >= MAX_ASM_OPERANDS)
        cprime_error("too many asm operands");
...
ST_FUNC void asm_instr(void)
{
      ...
      if (tok != ')')
      {
        ...
        if (tok == ':')
        {
          ...
          for (;;)
          {
            ...
            next();
            asmname = asm_get_prefix_name(cprime_state, "LG.",
                                          ++asmgoto_n);
            if (!csym->c)
  ...
  cprime_assemble_inline(cprime_state, astr.data, astr.size - 1, 0);
  cstr_free_s(&astr);
  if (sec != cur_text_section)
  {
    cprime_warning("inline asm tries to change current section");
    use_section1(cprime_state, sec);
  }
...
ST_FUNC int cprime_assemble(CPRIMEState *s1, int do_preprocess)
{
  cprime_error("asm not supported");
}

ST_FUNC void asm_instr(void)
{
  cprime_error("inline asm() not supported");
}
