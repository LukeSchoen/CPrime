...
/* The tree root is the nearest ancestor of the running tool that holds the
   published compiler, so a workflow executable keeps working wherever its own
   directory sits. */
static void nt_tree_root(char *out, size_t capacity) {
    char candidate[NT_PATH];
    int depth;
    nt_module_directory(out, capacity);
    for (depth = 0; depth < 16; ++depth) {
        nt_join(candidate, sizeof candidate, out, "cpc.exe");
        if (nt_exists(candidate)) return;
        nt_parent(out);
    }
    nt_die("cannot find the tree root (no cpc.exe above this tool)", out);
}

/* The toolchain directory holds the compiler's private include/ and lib/
   trees; it is what workflow tools pass to the compiler as -B. */
static void nt_toolchain_dir(char *out, size_t capacity) {
    char root[NT_PATH];
    nt_tree_root(root, sizeof root);
    nt_join(out, capacity, root, "src");
}
