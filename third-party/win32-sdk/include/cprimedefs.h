/*  cprimedefs.h
    ...
    and the few things related to option settings in cprimepp.c:cprime_predefs().

    This file is either included at runtime as is, or converted and
    included as C-strings at compile-time (depending on CONFIG_CPRIME_PREDEFS).

    Note that line indent matters:

    - in lines starting at column 1, platform macros are replaced by
      corresponding CPRIME target compile-time macros.  See conftest.c for
...
# endif
    ...
    #define _cprime_align(addr,type) (((unsigned)addr + _cprime_alignof(type) - 1) \
                                  & ~(_cprime_alignof(type) - 1))
    #define __builtin_va_start(ap,last) (ap = ((char *)&(last)) + ((sizeof(last)+3)&~3))
    #define __builtin_va_arg(ap,type) (ap = (void *) ((_cprime_align(ap,type)+sizeof(type)+3) \
    ...
    #define _cprime_align(addr,type) (((unsigned long)addr + __alignof__(type) - 1) \
                                  & -(__alignof__(type)))
    ...
    /* CPRIME BBUILTIN AND BOUNDS ALIASES */
