...
$result = Invoke-GccProcess ($arguments + @($source, '-O0', '-o', $exe, ('-Wl,-Map=' + $map))) 5
if ($result.exit -ne 0 -or $null -eq $result.exit) { throw "Map build failed: $($result.output)" }
$run = Invoke-GccProcess @($exe) 5
...
$failure = Invoke-GccProcess ($arguments + @($source, '-o', $exe, ('-Wl,-Map=' + $badMap))) 5
