...
static const char *runtime_sources[] = {
    ...
    "third-party\\quickjs-regexp\\cutils.c",
    "third-party\\quickjs-regexp\\libunicode.c",
    "third-party\\quickjs-regexp\\libregexp.c",
...
static void add_common_flags(NtBuffer *batch, const BuildPaths *paths, const char *runtime_root) {
    ...
    absolute_path(value, sizeof value, paths, "include/runtime"); {
        char flag[NT_PATH + 4]; snprintf(flag, sizeof flag, "-I%s", value); batch_arg(batch, flag);
    }
    absolute_path(value, sizeof value, paths, "include/cprime"); {
        char flag[NT_PATH + 4]; snprintf(flag, sizeof flag, "-I%s", value); batch_arg(batch, flag);
    }
    absolute_path(value, sizeof value, paths, "third-party/win32-sdk/include"); {
        char flag[NT_PATH + 4]; snprintf(flag, sizeof flag, "-I%s", value); batch_arg(batch, flag);
    }
    absolute_path(value, sizeof value, paths, "third-party/win32-sdk/include/winapi"); {
        char flag[NT_PATH + 4]; snprintf(flag, sizeof flag, "-I%s", value); batch_arg(batch, flag);
    }
...
static void copy_definitions(const BuildPaths *paths, const char *destination) {
    ...
    absolute_path(source_dir, sizeof source_dir, paths, "third-party/win32-sdk/lib");
...
static void add_runtime_jobs(NtBuffer *batch, const BuildPaths *paths,
    ...
    for (i = 0; i < (int)(sizeof runtime_sources / sizeof runtime_sources[0]); ++i) {
        ...
        if (!strcmp(runtime_names[i], "regex")) {
            absolute_path(source, sizeof source, paths, "third-party/quickjs-regexp");
        ...
        if (!strcmp(runtime_names[i], "exception") || !strcmp(runtime_names[i], "rtti") ||
            ...
            absolute_path(source, sizeof source, paths, "include/runtime");
...
static void initialize_paths(BuildPaths *paths) {
    ...
    strcpy(paths->root, module);
    nt_parent(paths->root);
    nt_join(paths->cpc, sizeof paths->cpc, paths->root, "cpc.exe");
    nt_join(paths->build, sizeof paths->build, paths->root, "build");
...
static void overlay_runtime_headers(const BuildPaths *paths, const char *stage_include) {
    ...
    absolute_path(runtime, sizeof runtime, paths, "include\\runtime");
...
static int prepare_package(const BuildPaths *paths) {
    ...
    absolute_path(source, sizeof source, paths, "third-party\\win32-sdk\\include");
    snprintf(sdk_flag, sizeof sdk_flag, "-I%s", source);
    absolute_path(source, sizeof source, paths, "third-party\\win32-sdk\\include\\winapi");
    ...
    absolute_path(source, sizeof source, paths, "third-party\\win32-sdk\\lib");
...
static int validate_candidate(const BuildPaths *paths) {
    ...
    NtProcessResult result;
    nt_join(runner, sizeof runner, paths->root, "Tests/test.exe");
