...
static int compiler_arguments(const char **args, const char *compiler,
    ...
    int n = 0;
    snprintf(inc1,sizeof inc1,"-I%s\\include\\cprime",root);
    ...
    snprintf(runtime_inc,sizeof runtime_inc,"-I%s\\include\\runtime",root);
    snprintf(sdk_inc,sizeof sdk_inc,"-I%s\\third-party\\win32-sdk\\include",root);
    snprintf(winapi_inc,sizeof winapi_inc,"-I%s\\third-party\\win32-sdk\\include\\winapi",root);
...
int main(int argc, char **argv) {
    ...
    nt_join(tcc,sizeof tcc,root,"third-party\\tcc\\win32\\tcc.exe");
    nt_join(out,sizeof out,root,"build\\seed-tcc");
    for(i=1;i<argc;i++) {
    ...
    nt_join(test_source,sizeof test_source,root,"Tests\\c_compat\\pass\\test_basic_arithmetic.c");
