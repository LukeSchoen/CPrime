...
try {
    ...
    $plain = Invoke-TestProcess ($command + @($source, '-c', '-o', (Join-Path $work 'plain.o'))) 5
    $env:CPRIME_PARSER_STATE = '1'
    $diagnostic = Invoke-TestProcess ($command + @($source, '-c', '-o', (Join-Path $work 'state.o'))) 5
    ...
    $result = Invoke-TestProcess ($command + @($recovery, '-c', '-o', (Join-Path $work 'recovery.o'))) 5
