/* Scan the first-party compiler sources for leftovers that make compilation
   ...
   Usage: perf-check [options]
     ...
     -Baseline FILE   baseline TSV (default ROOT/Cost/baseline/checks.tsv)
     -Out FILE        current counts TSV (default ROOT/Cost/build/perf/perf-checks.tsv)
...
int main(int argc, char **argv)
{
  ...
  if (!baseline_path[0]) join_path(baseline_path, sizeof baseline_path, root, "Cost\\baseline\\checks.tsv");
  if (!out_path[0]) join_path(out_path, sizeof out_path, root, "Cost\\build\\perf\\perf-checks.tsv");
