...
static inline int get_reg_shift(CPRIMEState *s1)
{
  int shift, v;
  v = asm_int_expr(s1);
  switch (v)
...
static void parse_operand(CPRIMEState *s1, Operand *op)
{
  ExprValue e;
  int reg, indir;
  const char *p;
  ...
  if (tok == '%')
  {
    ...
    {
reg_error:
      cprime_error("unknown register %%%s", get_tok_str(tok, &tokc));
    }
    next();
no_skip: ;
  }
...
static void gen_disp32(ExprValue *pe)
{
  ...
  if (esym && esym->st_shndx == cur_text_section->sh_num)
  {
    /* same section: we can output an absolute value. Note
       that the CPRIME compiler behaves differently here because
                    ...
                    int regi, int rmi)
{
  ...
  {
    if (saw_high_8bit)
      cprime_error("can't encode register %%%ch when REX prefix is required",
...
ST_FUNC void asm_opcode(CPRIMEState *s1, int opcode)
{
  ...
  // One-Line Things Like "Rep Stosb" Instead Of Only "Rep\Nstosb"
  ...
  for (;;)
  {
    ...
    if (nb_ops >= MAX_OPERANDS)
      cprime_error("incorrect number of operands");
    parse_operand(s1, pop);
    if (tok == ':')
    {
      if (!(pop->type & OP_SEG) || seg_prefix)
        cprime_error("incorrect prefix");
      seg_prefix = segment_prefixes[pop->reg];
      next();
      parse_operand(s1, pop);
      if (!(pop->type & OP_EA))
        cprime_error("segment prefix must be followed by memory reference");
    }
    pop++;
    nb_ops++;
    if (tok != ',')
...
next: ;
  ...
  if (pa->sym == 0)
  {
    ...
    {
      cprime_error("bad operand with opcode '%s'",
    ...
    else
    {
      ...
      cprime_error("unknown opcode '%s'", ts->str);
    }
  }
  ...
  if (s == autosize)
  {
    ...
    if (s == autosize)
    {
      if ((opcode == TOK_ASM_push || opcode == TOK_ASM_pop) &&
        ...
        cprime_error("cannot infer opcode suffix");
    }
  }
  ...
  default: cprime_error("bad prefix 0x%2x in opcode table", p); break;
  }
  ...
  if (pa->instr_type & OPC_MODRM)
  {
    if (!nb_ops)
    {
      ...
      if (pa->sym == TOK_ASM_endbr32)
        ...
        cprime_error("bad MODR/M opcode without operands");
      goto modrm_found;
    }
...
no_short_jump:
      ...
      if (v == 0xeb) // Jmp
        ...
        cprime_error("invalid displacement");
    }
  }
  ...
  for (i = 0; i < nb_operands; i++)
  {
    ...
    if (isnum(*str) || *str == '[')
    {
      ...
      if ((unsigned)k >= i || i < nb_outputs)
        cprime_error("invalid reference in constraint %d ('%s')",
                  i, str);
      op->ref_index = k;
      if (operands[k].input_index >= 0)
        cprime_error("cannot reference twice the same operand");
  ...
  for (i = 0; i < nb_operands; i++)
  {
    ...
    if (op->reg >= 0)
    {
      if (is_reg_allocated(op->reg))
        cprime_error("asm regvar requests register that's taken already");
      reg = op->reg;
    }
try_next:
    c = *str++;
    ...
    case '&':
      if (j >= nb_outputs)
        cprime_error("'%c' modifier can only be applied to outputs", c);
      reg_mask = REG_IN_MASK | REG_OUT_MASK;
      goto try_next;
    case 'A':
...
reg_found1:
      ...
      break;
    default:
      cprime_error("asm constraint %d ('%s') could not be satisfied",
                j, op->constraint);
      break;
    }
  ...
  *pout_reg = -1;
  for (i = 0; i < nb_operands; i++)
  {
        ...
        !op->is_memory)
    {
      ...
      cprime_error("could not find free output register for reloading");
reg_found2:
      *pout_reg = reg;
      break;
    }
...
no_offset:;
  ...
  else
  {
    ...
    {
      if (reg >= 4)
        cprime_error("cannot use byte register");
      size = 1;
    }
    else if (modifier == 'h')
    {
...
ST_FUNC void asm_clobber(uint8_t *clobber_regs, const char *str)
{
  ...
  else if ((reg = asm_parse_numeric_reg(reg, &type)) >= 0)
  {
    ...
    cprime_error("invalid clobber register '%s'", str);
  clobber_regs[reg] = 1;
}
