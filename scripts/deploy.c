...
int main(int argc, char **argv) {
    ...
    nt_join(staged, sizeof staged, root, "build\\deploy\\libcprime.dll");
    { char staging[NT_PATH]; nt_join(staging, sizeof staging, root, "build\\deploy"); nt_mkdirs(staging); }
    DeleteFileA(staged);
    compile[n++] = cpc;
#define ADD_INCLUDE(relative) do { nt_join(source, sizeof source, root, relative); snprintf(flag, sizeof flag, "-I%s", source); compile[n++] = nt_strdup(flag); } while (0)
    ...
    nt_join(cache, sizeof cache, root, "build\\portable-cache");
    nt_join(packager, sizeof packager, cache, "portable-payload.exe");
    nt_join(payload, sizeof payload, cache, "payload.bin");
    nt_join(runtime_lib, sizeof runtime_lib, root, "build\\compiler\\lib");
    if (!nt_exists(packager) || !nt_exists(payload))
        nt_die("portable payload is not prepared; run scripts\\build.exe first", cache);
    ...
    if (!result.started || result.timed_out || result.exit_code) {
        ...
        printf("WARN: cached portable payload is stale; run scripts\\build.exe to refresh it\n");
    }
    nt_process_free(&result);
