...
{
  Material material;
  material.name.assign(nameValue);
  material.texture.assign(textureValue);
  return material;
}
...
int main(void)
{
  ...
  if (!materials.items[0].texture.equals("texture.png"))
    return 3;
  if (!materials.items[11].name.equals("mat"))
    return 4;
  if (!materials.items[11].texture.equals("texture.png"))
    return 5;
  return 0;
}
