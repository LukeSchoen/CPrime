...
#include "../../src/scripts/common/native_tool.h"
...
#define A(x) args[n++] = (x)
    ...
    if (mode < 2) {
        snprintf(bflag, sizeof bflag, "-B%s\\src", root); A(bflag); A("-O2");
        A("-Isrc/include/runtime");
        A("-Isrc/include/cprime"); A("-Isrc/third-party/win32-sdk/include");
        A("-Isrc/third-party/win32-sdk/include/winapi");
    } else {
        A("-O0"); A("-g0");
        if (c->cpp) { A("-std=c++17"); }
    ...
    if (c->self) {
        A("-Isrc/include/cprime"); A("-Isrc/compiler/frontend"); A("-Isrc/compiler/middleend");
    ...
    if (ok && phase == 2 && c->self) {
        ...
        snprintf(bflag,sizeof bflag,"-B%s\\src",root);
        execute[e++]=output; execute[e++]=bflag; execute[e++]="-O2";
        execute[e++]="-Isrc/include/runtime"; execute[e++]="-Isrc/include/cprime";
        execute[e++]="-Isrc/third-party/win32-sdk/include";
        execute[e++]="-Isrc/third-party/win32-sdk/include/winapi";
