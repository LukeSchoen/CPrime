...
rem levels. Without optimization flags the job is to compile as fast as
rem possible while keeping cheap, effective speedups that cost no compile time.
rem With optimization flags (-O1/-O2/-Os) the compiler may spend compile time to
rem take on much more, aiming at the runtime speed of the equivalent clang, gcc
...
rem   PERF_ARGS        per-cycle compile-time budget measurement (default below)
...
rem
rem Logs live in build\worker\strength: cycles.csv has one row per cycle,
...
if not exist "%ROOT%\cpc.exe" (
    echo [%DATE% %TIME%] no root cpc.exe; compile-time measurement skipped.
    exit /b 0
)
if not exist "%ROOT%\scripts\performance.exe" (
    echo [%DATE% %TIME%] no scripts\performance.exe; compile-time measurement skipped.
    exit /b 0
)
"%ROOT%\scripts\performance.exe" -Root "%ROOT%" %PERF_ARGS% -Results "%MEASURE_OUT%" > "%MEASURE_LOG%" 2>&1
set "MEASURE_RC=!ERRORLEVEL!"
if "!MEASURE_RC!"=="0" (
    echo [%DATE% %TIME%] compile-time budget: %MEASURE_OUT%
) else (
